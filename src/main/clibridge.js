...
function searchPath(name) {
  ...
  const exts = process.platform === 'win32' ? [ '', ...String(process.env.PATHEXT || '.COM;.EXE;.BAT;.CMD;.PS1').split(';').filter(Boolean) ] : [ '' ];
...
function probeCommand(command) {
  try {
    ...
    return {
      ok: true,
      path: r.file === (process.env.ComSpec || 'cmd.exe') || r.file === 'powershell.exe' ? r.prefixArgs[r.prefixArgs.length - 1] : r.file,
      kind: r.why
    };
  } catch (e) {
    return {
...
function flatten(messages) {
  ...
  for (const m of messages || []) {
    ...
    for (const piece of content || []) {
      if (!piece) continue;
      if (piece.type === 'text') parts.push(piece.text || ''); else if (piece.type === 'image_url') {
...
const stripAnsi = s => String(s || '').replace(/\[[0-9;?]*[ -/]*[@-~]/g, '').replace(/\r/g, '');
...
function runCli(endpoint, messages, {timeoutSec: timeoutSec} = {}) {
  const {prompt: prompt, images: images} = flatten(messages);
  const seconds = Number(timeoutSec || endpoint.timeoutSec) || 300;
  const viaStdin = (endpoint.promptVia || 'stdin') === 'stdin';
  const template = Array.isArray(endpoint.args) && endpoint.args.length ? endpoint.args : endpoint.args ? String(endpoint.args).split(/\s+/).filter(Boolean) : [];
  const args = [];
  ...
  return new Promise((resolve, reject) => {
    ...
    child.on('close', code => {
      ...
      resolve({
        ...
        promptTokens: 0,
        completionTokens: 0,
        stdout: stripAnsi(out).slice(0, 4e3),
        stderr: stripAnsi(err).slice(0, 2e3),
        exitCode: code
      });
    });
  });
