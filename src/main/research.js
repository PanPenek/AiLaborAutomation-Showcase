...
function decodeHtml(value) {
  return String(value || '').replace(/&#x([0-9a-f]+);/gi, (_m, n) => String.fromCodePoint(parseInt(n, 16))).replace(/&#(\d+);/g, (_m, n) => String.fromCodePoint(Number(n))).replace(/&(amp|quot|apos|lt|gt|nbsp|#39);/gi, (m, n) => ({
    amp: '&',
    quot: '"',
    apos: "'",
    lt: '<',
...
function stripMarkup(value, max = 600) {
  return decodeHtml(String(value || '').replace(/<script\b[\s\S]*?<\/script>/gi, ' ').replace(/<style\b[\s\S]*?<\/style>/gi, ' ').replace(/<br\s*\/?\s*>/gi, ' ').replace(/<[^>]+>/g, ' ')).replace(/[\u0000-\u001f\u007f]+/g, ' ').replace(/\s+/g, ' ').trim().slice(0, max);
}
...
function parseDuckLite(html, limit = 8) {
  ...
  for (let i = 0; i < anchors.length && out.length < limit; i++) {
    ...
    out.push({
      title: title,
      url: href,
      snippet: stripMarkup(snippet, 520),
      provider: 'DuckDuckGo'
    });
  }
  return out;
...
function cleanMarkdown(value, max = 900) {
  return stripMarkup(String(value || '').replace(/\[([^\]]+)\]\([^)]+\)/g, '$1').replace(/[_*~`>#]+/g, ' '), max);
}
...
async function requestKnown(url, {method: method = 'GET', headers: headers = {}, body: body = null, timeoutMs: timeoutMs = 2e4, maxBytes: maxBytes = JSON_LIMIT} = {}, deps = {}) {
...
async function requestRemote(raw, {headers: headers = {}, timeoutMs: timeoutMs = 25e3, maxBytes: maxBytes = HTML_LIMIT} = {}, deps = {}) {
  const fetchImpl = deps.fetch || global.fetch;
  let current = String(raw || '');
  for (let hop = 0; hop < 5; hop++) {
    if (!deps.skipDns) await assertPublicDns(current, deps); else if (!isSafeRemoteUrl(current)) throw new Error('refused a local or invalid web address');
    const controller = new AbortController;
    const timer = setTimeout(() => controller.abort(), timeoutMs);
    try {
...
async function searchDuck(query, limit, deps) {
  ...
  const {bytes: bytes} = await requestKnown(url, {
    headers: {
      'User-Agent': BROWSER_UA,
...
async function searchWikipedia(query, limit, deps) {
  ...
  const {bytes: bytes} = await requestKnown(`https://en.wikipedia.org/w/api.php?${p}`, {}, deps);
  const data = JSON.parse(bytes.toString('utf8'));
  return Object.values(data?.query?.pages || {}).sort((a, b) => (a.index || 0) - (b.index || 0)).slice(0, limit).map(page => ({
...
function characterTerms(query) {
  ...
  return [ ...new Set([ q, q.split(/\s+from\s+/i)[0].trim(), q.replace(/\s*\([^)]*\)\s*$/, '').trim() ].filter(Boolean)) ];
}
...
async function searchAniList(query, deps) {
  ...
  for (const term of characterTerms(query)) {
    try {
      const {bytes: bytes} = await requestKnown('https://graphql.anilist.co', {
        method: 'POST',
        headers: {
      ...
      return {
        ...
        image: {
          title: `${c.name.full} character portrait`,
          imageUrl: c.image?.large || c.image?.medium || '',
          thumbnailUrl: c.image?.medium || c.image?.large || '',
          sourceUrl: sourceUrl,
          provider: 'AniList',
          creator: '',
          license: '',
...
async function searchOpenverse(query, limit, deps) {
  ...
  const {bytes: bytes} = await requestKnown(`https://api.openverse.org/v1/images/?${p}`, {}, deps);
...
async function searchCommons(query, limit, deps) {
  ...
  const {bytes: bytes} = await requestKnown(`https://commons.wikimedia.org/w/api.php?${p}`, {}, deps);
...
async function mediaWikiImage(source, deps) {
  ...
  const {bytes: bytes} = await requestKnown(`${u.origin}${apiPath}?${p}`, {}, deps);
  ...
  return {
    ...
    imageUrl: image,
    thumbnailUrl: page?.thumbnail?.source || image,
...
async function pageMetaImage(source, deps) {
  const {bytes: bytes, url: url} = await requestRemote(source.url, {
    headers: {
...
async function discoverPageImage(source, deps) {
  ...
  try {
    return await pageMetaImage(source, deps);
  } catch {
    return null;
  }
}
...
function imageRelevance(row, query, kind, index) {
  ...
  if (/\b(wholesale|buy|shop|shopping|coupon|vacuum cleaner|product listing)\b/.test(title) || /\b(alibaba|aliexpress|amazon|ebay|temu)\./.test(title)) score -= 80;
...
async function search(payload = {}, deps = {}) {
  ...
  if (!query) return {
    ok: false,
    error: 'web research needs a query',
    sources: [],
    images: []
  };
  ...
  const searchQuery = kind === 'character' ? `${query} character appearance outfit reference` : `${query} visual reference design motifs`;
  const warnings = [];
  const webTask = searchDuck(searchQuery, webLimit, deps).catch(e => {
    warnings.push(`DuckDuckGo: ${e.message}`);
    return [];
  });
  const specialistTask = kind === 'character' ? searchAniList(query, deps).catch(e => {
    warnings.push(`AniList: ${e.message}`);
    return null;
  }) : searchOpenverse(query, imageLimit, deps).catch(e => {
    warnings.push(`Openverse: ${e.message}`);
    return [];
  });
  ...
  if (sources.length < 3) {
    const wiki = await searchWikipedia(query, webLimit, deps).catch(e => {
      warnings.push(`Wikipedia: ${e.message}`);
      return [];
    });
  ...
  if (images.length < imageLimit) {
    const commons = await searchCommons(query, imageLimit, deps).catch(e => {
      warnings.push(`Wikimedia Commons: ${e.message}`);
      return [];
    });
    images.push(...commons);
  }
  const finalSources = dedupeSources(sources).slice(0, webLimit).map((r, i) => ({
    id: `s${i + 1}`,
    title: r.title,
    url: r.url,
    snippet: r.snippet || '',
  ...
  const rankedImages = dedupeImages(images).map((row, index) => ({
    row: row,
    score: imageRelevance(row, query, kind, index)
  })).sort((a, b) => b.score - a.score).slice(0, imageLimit).map(({row: row}) => row);
  const finalImages = rankedImages.map((r, i) => {
    ...
    return {
      ...
      sourceUrl: isSafeRemoteUrl(r.sourceUrl) ? r.sourceUrl : '',
      provider: r.provider || 'Web',
      creator: r.creator || '',
      license: r.license || '',
      width: r.width || null,
  ...
  if (!finalSources.length && !finalImages.length) {
    return {
      ok: false,
      error: warnings[0] || 'no web results or reference images were found',
      query: query,
      kind: kind,
      sources: [],
      images: [],
...
async function fetchImageBounded(payload = {}, deps = {}, {maxBytes: maxBytes = IMAGE_LIMIT, thumbnailFirst: thumbnailFirst = false} = {}) {
  const urls = thumbnailFirst ? [ payload.thumbnailUrl, payload.imageUrl || payload.url ] : [ payload.imageUrl || payload.url, payload.thumbnailUrl ];
  ...
  for (const url of choices) {
    try {
      const headers = {
        Accept: 'image/avif,image/webp,image/png,image/jpeg,image/gif,*/*;q=0.4'
      };
      if (isSafeRemoteUrl(payload.sourceUrl)) headers.Referer = payload.sourceUrl;
      const {response: response, bytes: bytes, url: finalUrl} = await requestRemote(url, {
        headers: headers,
        timeoutMs: 35e3,
        maxBytes: maxBytes
      }, deps);
  ...
  return {
    ok: false,
    error: errors.join(' | ').slice(0, 1e3) || 'the reference image could not be downloaded'
  };
}

async function fetchImage(payload = {}, deps = {}) {
  return fetchImageBounded(payload, deps, {
...
module.exports = {
  ...
  parseDuckLite: parseDuckLite,
  parseOpenverse: parseOpenverse,
  parseCommons: parseCommons,
  metaMap: metaMap,
  isPrivateIp: isPrivateIp,
  isSafeRemoteUrl: isSafeRemoteUrl,
  sniffMime: sniffMime,
  _internals: {
