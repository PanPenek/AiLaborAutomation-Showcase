(function() {
  ...
  const Teach = {
    manual() {
      ...
      if (!m.rules || typeof m.rules !== 'object') m.rules = {
        always: [],
        never: []
      };
      ...
      if (!m.boost || typeof m.boost !== 'object') m.boost = {
        themes: {},
        tags: {}
      };
    ...
    addLesson(text, {why: why = '', weight: weight = 'prefer'} = {}) {
    ...
    isPinned(text) {
      return this.manual().pinned.includes(fingerprint(text));
    },
    isMuted(text) {
      return this.manual().muted.includes(fingerprint(text));
    },
    pin(text, why = '') {
      const m = this.manual();
    ...
    mute(text) {
      ...
      if (window.Insights && Insights.playbook) {
        Insights.playbook.lessons = (Insights.playbook.lessons || []).filter(l => fingerprint(l.lesson) !== f);
      }
      this._save();
      return m;
    },
    ...
    setRules({always: always, never: never} = {}) {
      const m = this.manual();
      const clean = arr => (Array.isArray(arr) ? arr : String(arr || '').split('\n')).map(x => String(x).trim()).filter(Boolean).slice(0, 20).map(x => x.slice(0, 240));
    ...
    setBanned(list) {
      const m = this.manual();
      m.banned = (Array.isArray(list) ? list : String(list || '').split(/[\n,]/)).map(x => String(x).trim().toLowerCase()).filter(Boolean).filter((x, i, a) => a.indexOf(x) === i).slice(0, 60).map(x => x.slice(0, 80));
      this._save();
      return m.banned;
    },
    setNotes(text) {
      const m = this.manual();
      m.notes = String(text || '').slice(0, 2e3);
      this._save();
      return m.notes;
    },
    setBoost(kind, key, mult) {
      ...
      if (!k) return m.boost;
      const n = Number(mult);
      if (!Number.isFinite(n) || n === 1) delete bucket[k]; else bucket[k] = Math.max(0, Math.min(5, n));
      this._save();
      return m.boost;
    },
    violations(prompt) {
    ...
    isEmpty() {
      const m = this.manual();
      return !m.lessons.length && !m.rules.always.length && !m.rules.never.length && !m.banned.length && !m.notes.trim() && !Object.keys(m.boost.themes).length && !Object.keys(m.boost.tags).length;
    },
    count() {
      const m = this.manual();
    ...
    async propose(text) {
      ...
      return {
        lessons: (Array.isArray(j.lessons) ? j.lessons : []).map(l => ({
    ...
    applyProposal(p) {
      const m = this.manual();
      for (const l of p.lessons || []) this.addLesson(l.text, {
        why: l.why,
        weight: l.weight
      });
      if ((p.always || []).length) this.setRules({
        always: [ ...m.rules.always, ...p.always ]
      });
      if ((p.never || []).length) this.setRules({
        never: [ ...this.manual().rules.never, ...p.never ]
      });
