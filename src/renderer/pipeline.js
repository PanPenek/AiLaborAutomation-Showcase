(function() {
  ...
  const descriptionHeadroom = (count = 1) => ((State.settings || {}).metadata || {}).expandedStorytelling ? 900 * count : 0;
  ...
  const noDisclaimerRule = () => ageDisclaimer() ? '\n- Do NOT write any disclaimer line. That line is' + ' appended automatically underneath and a second one in your own words reads as a stutter.' : '';
  const ALTS_RULE = `2. "alts": TWO more titles for this same image, same rules, ranked best first. ` + `They are used only if the first one turns out to be already taken in this gallery, ` + `so they must be genuinely DIFFERENT ideas — a different image in the mind, a different ` + `angle on the moment — not the first title reworded.`;
  ...
  function fingerReport(str, {handsVerdict: handsVerdict = null} = {}) {
    const s = String(str || '');
    ...
    for (const seg of segs) {
      ...
      let count = null;
      if (range) count = null; else {
        const n = seg.match(/\d+/);
        if (n) count = Number(n[0]) + (FINGER_PLUS_THUMB_RE.test(seg) ? 1 : 0);
      }
      const visibleSubset = count !== null && count < 5 && /\bvisible\b/i.test(seg) && (region === 'CLEAN' || region === 'NOT_VISIBLE');
      let status;
      if (bad) status = 'defect'; else if (hidden) status = 'hidden'; else if (range || visibleSubset || FINGER_UNCOUNTABLE_RE.test(seg) || /\b(?:uncertain|unsure|maybe|approximately)\b|[~?]/i.test(seg)) status = 'unreadable'; else if (count !== null && count !== 5) status = 'defect'; else if (count === 5) status = 'clean'; else status = 'unreadable';
      hands.push({
        side: side,
        status: status,
        said: seg
    ...
    return {
      hands: hands,
      bad: hands.filter(h => h.status === 'defect')
    };
  }
  const SEVERE_BAND = 4;
  ...
  const qcScoreNumber = v => (typeof v === 'number' || typeof v === 'string' && /^\d+(?:\.\d+)?$/.test(v.trim())) && Number.isFinite(Number(v)) && Number(v) >= 1 && Number(v) <= 10;
  function validateQc(qc) {
    const invalid = field => {
      throw new Error(`Invalid QC response: ${field}`);
    };
  ...
  function gateScore(gates) {
    ...
    let ceiling;
    if (structural.length) ceiling = GATE_CEILING[Math.min(structural.length, 3)]; else if (no) ceiling = DETAIL_ONLY_CEILING; else ceiling = 10;
    ...
    return {
      ...
      failed: failed,
      answers: answers,
      complete: answered === GATE_KEYS.length && hasOverall,
  ...
  const T = {
    ideation(theme, example, count, avoid = [], guidance = '', extras = {}) {
      const {house: house = '', skeleton: skeleton = '', modeRule: modeRule = '', extra: extra = ''} = extras;
    ...
    metadata({prompt: prompt, exampleStyle: exampleStyle, maxTags: maxTags, variety: variety = '', scene: scene = ''}) {
    ...
    metadataBatch({prompt: prompt, count: count, exampleStyle: exampleStyle, maxTags: maxTags, variety: variety = '', preferredTags: preferredTags = [], titleHints: titleHints = [], scenes: scenes = []}) {
  ...
  function stripPatreonBlock(text, {link: link, cta: cta} = {}) {
  ...
  function addPatreonBlock(text, {link: link, cta: cta} = {}) {
    const body = stripPatreonBlock(text, {
      link: link,
      cta: cta
    });
    return `${link || ''}\n\n${cta || ''}\n\n${body}`.trim();
  ...
  function destinationsOf(card) {
    const raw = card && Array.isArray(card.destinations) ? card.destinations : card && isDestination(card.destination) ? [ card.destination ] : null;
  ...
  function destinationOf(card) {
    ...
    return list.includes('patreon') ? 'patreon' : list.includes('deviantart') ? 'deviantart' : list[0];
  }
  ...
  function settleTitles(cards) {
    ...
    for (const c of cards) {
      ...
      const takenByAnother = (State.library || []).some(o => o && o.id !== c.id && o.metadata && same(o.metadata.title));
      const verdict = Titles.isRepeat(title, {
        exclude: [ title ]
      });
      ...
      if (!verdict.repeat && !takenByAnother && !clashesWithSibling) {
        ...
        const against = takenByAnother ? title : (verdict.against || {}).t || 'another title in this batch';
        const pick = Titles.pickFree(alts, {
          avoid: [ ...chosen, title ]
        });
        if (pick) {
          meta.title = pick.title;
          meta.titleWritten = pick.title;
          c.titleWarning = `Written as "${title}", which is close to "${against}". ` + `Swapped for the writer's own next choice — no extra call was made.`;
        } else {
          c.titleWarning = alts.length ? `Close to "${against}", and so were the spare titles. Kept as written — rename it here if you disagree.` : `Close to "${against}". Kept as written — rename it here if you disagree.`;
        }
      }
  ...
  function hasMetadata(card) {
    ...
    if (!m) return false;
    return !!(String(m.title || '').trim() || String(m.description || '').trim() || Array.isArray(m.tags) && m.tags.length);
  }
  ...
  const RETRY_BACKOFF_MS = [ 3e4, 12e4, 48e4 ];
  const Pipeline = {
    running: false,
    ...
    attachDriver(webviewEl) {
      this.driver = new PerchanceDriver(webviewEl);
    },
    genDriver() {
    ...
    _cacheReference(key, value) {
      const bytes = Math.max(0, Number(value.bytes) || Math.floor(String(value.base64 || '').length * .75));
    ...
    async researchReferences(job) {
      const limit = window.ComfyUI?.referenceLimit ? window.ComfyUI.referenceLimit(State.settings && State.settings.comfy) : 8;
      const ids = [ ...new Set((Array.isArray(job.referenceIds) ? job.referenceIds : []).map(String).filter(Boolean)) ].slice(0, limit);
      if (!ids.length) return [];
      const chatById = new Map((Array.isArray(job.chatReferences) ? job.chatReferences : []).filter(r => r && r.id && r.fname).map(r => [ String(r.id), r ]));
      const loaded = [];
      const failed = [];
      for (const id of ids.filter(x => chatById.has(x))) {
        ...
        try {
          ...
          loaded.push({
            id: id,
            key: `chat:${row.fname}`,
            title: String(row.name || id).slice(0, 240),
            base64: got.base64,
            mime: got.mime || row.mime || 'image/png',
            bytes: 0
          });
        } catch (e) {
          failed.push({
            id: id,
            error: `attached image unavailable: ${e.message}`
          });
        }
      }
      const researchIds = ids.filter(x => !chatById.has(x));
      const pack = researchIds.length && window.Overseer && typeof window.Overseer.researchPack === 'function' ? window.Overseer.researchPack(job.researchId) : null;
      if (researchIds.length && !pack) {
        failed.push(...researchIds.map(id => ({
          id: id,
          error: 'saved research pack is no longer available'
        })));
      ...
      for (const id of pack ? researchIds : []) {
        ...
        if (!image) {
          failed.push({
            id: id,
            error: 'reference id is not in the saved research pack'
          });
          continue;
        }
      ...
      log(loaded.length ? `Prepared ${loaded.length}/${ids.length} reference image(s) for the ComfyUI workflow.` : 'No searched reference image could be downloaded — generating from the already grounded prompt text.', loaded.length ? undefined : 'err');
      return loaded;
    },
    parallelPostOn() {
    ...
    laneImages() {
      ...
      const inFlight = this._laneCurrent ? this._laneCurrent.items.filter(it => it.card.status === 'qc').length : 0;
    ...
    controlMenu() {
      ...
      const lines = [ ...groups.values() ].sort((a, b) => rank(a) - rank(b)).map(g => {
        ...
        const head = g.names.length > 1 ? `${g.names[0]} … ${g.names[g.names.length - 1]} (${g.names.length} slots, all offering this same list)` : `${g.names[0]} (${g.label})`;
        return `- ${head}: ${shown.join(' | ')}` + (more > 0 ? ` … and ${more} more not listed` : '');
      });
    ...
    async chooseControls(prompt) {
      ...
      const r = await U.llmChat([ {
        role: 'user',
        content: T.controlsFor(prompt, menu, maxControls, this.pinnedStyle())
      } ], {
        temperature: .3,
        maxTokens: 4e3,
        role: 'ideation'
      }, 'Choosing generator controls');
      let obj = null;
    ...
    async ideate(theme, example, count, avoid = [], guidance = null, mode = '') {
      const learned = guidance == null ? window.Insights ? window.Insights.guidance({
        theme: theme
      }) : '' : guidance;
      ...
      if (!seed && !house && window.PromptStyle) {
        const best = PromptStyle.exemplars({
          n: 1
        })[0];
        if (best) seed = best.text;
      }
      const skeletonBlock = window.PromptLab ? PromptLab.skeletonBlock(await this.themeSkeleton(seed)) : '';
      ...
      if (this.advancedIdeationOn() && this.driver && this.genDriver() === this.driver) {
        ...
        if (!menu) {
          log('Advanced mode is on but the generator\'s controls have not been read yet —' + ' writing prompts without them. Press "Read the generator\'s options" in Settings → Perchance.', 'err');
        }
      }
      const advanced = !!menu;
      const maxControls = Math.max(1, Number((State.settings.gen || {}).advMaxFilters) || 12);
      ...
      const build = (n, dodge, extra) => advanced ? T.ideationAdvanced(theme, seed, n, dodge, learned, menu, maxControls, this.pinnedStyle(), {
        ...extras,
        extra: extra
      }) : T.ideation(theme, seed, n, dodge, learned, {
        ...extras,
        extra: extra
      });
      const ask = async (n, dodge, extra = '') => {
        const res = await U.llmChat([ {
          role: 'user',
          content: build(n, dodge, extra)
        } ], {
          temperature: .95,
          maxTokens: advanced ? 12e3 : 8e3,
          role: 'ideation'
        }, 'Ideation');
        return {
          res: res,
          ideas: advanced ? U.ideasFrom(res, {
            count: n,
            max: 600
          }) : U.promptsFrom(res, {
            count: n,
      ...
      if (!list.length) {
        throw new Error(r.truncated || r.fromReasoning ? `${r.provider || 'the writer'} used its whole token budget thinking and never emitted a prompt` + ' — raise its Max output tokens in Settings → Engines, or route ideation to a non-reasoning model' : 'LLM returned no prompts');
      }
      ...
      if (sim && list.length) {
        ...
        const fresh = (ideas, against) => {
          ...
          for (const i of ideas) {
            ...
            if (others.some(q => sim(i.prompt, q) > .82)) continue;
            kept.push(i);
          }
          return kept;
        };
        ...
        if (copies) {
          ...
          let repl = [];
          try {
            const again = await ask(copies, [ ...list.map(i => i.prompt), ...avoid || [] ], 'The previous attempt repeated scenes that already exist. Every prompt must be a' + ' clearly different scene from all of the prompts listed above — not a reworded one.');
      ...
      if (window.PromptStyle) {
        ...
        if (weak.length) {
          ...
          let repl = [];
          try {
            const again = await ask(weak.length, [ ...weak.map(i => i.prompt), ...avoid ], 'The previous attempt was rejected for form. Comma-separated fragments in the' + ' artist’s voice shown above: no prose, no third-person narration' + ' ("she is holding…"), and never open by naming the medium.');
            repl = again.ideas.filter(i => score(i) > 0);
          } catch (e) {
            log(`Could not re-ask for the off-voice prompts (${e.message}) — keeping them.`, 'err');
          }
          list = [ ...list.filter(i => score(i) > 0), ...repl, ...weak.slice(repl.length) ].slice(0, count);
          if (repl.length) log(`Replaced ${repl.length} of them.`, 'ok');
        }
        const sorted = list.map(score).sort((a, b) => a - b);
        log(`Voice score for this batch: median ${sorted[Math.floor(sorted.length / 2)]}` + ` (his own prompts: median 6 on this scale).`);
      }
      if (advanced) {
    ...
    async start() {
      ...
      if (!this.driver) {
        log('Perchance driver not attached yet.', 'err');
        return;
      }
      this.running = true;
      ...
      const tick = setInterval(() => {
        ...
        if (t && State.worker.statusText !== t) State.setWorker({
          statusText: t
        });
      }, 5e3);
      let jobsDone = 0;
        ...
        while (!this._stopRequested) {
          ...
          if (!job) {
            if (jobsDone > 0 && !this.laneImages()) {
              ...
              osNotify('AiLabor — queue finished', `${jobsDone} job(s) done. ${waiting} card(s) waiting in Review.`);
              jobsDone = 0;
            }
            this._phase = null;
            State.setWorker({
              currentJobId: null,
              statusText: this._phaseText() || 'Idle — queue empty'
            });
            await U.sleep(4e3);
            continue;
          }
          ...
          if (delaySec > 0 && !this._stopRequested) {
            ...
            await U.sleep(delaySec * 1e3);
          }
        }
      } finally {
        if (this._lanePromise && this.laneImages()) {
          this.setPhase(null);
          log(`Paused — finishing QC on ${this.laneImages()} image(s) already generated. ` + `Press Pause again to park them for later instead.`);
        }
    ...
    async awaitEngine() {
      ...
      while (this.engineDown && !this._stopRequested) {
        if (Date.now() - probedAt >= 3e4) {
          probedAt = Date.now();
          const st = await driver.status().catch(() => ({
            up: false
          }));
          if (st && st.up) {
            const mins = Math.round((Date.now() - this.engineDown.since) / 6e4);
            log(`Image engine is answering again${mins ? ` after ${mins} min` : ''} — resuming the queue.`, 'ok');
            this.engineDown = null;
            return;
          }
          if (String(cfg.launchCommand || '').trim() && Date.now() - (this.engineDown.launchTriedAt || 0) > 10 * 6e4) {
            this.engineDown.launchTriedAt = Date.now();
            return;
          }
        }
        const mins = Math.floor((Date.now() - this.engineDown.since) / 6e4);
    ...
    async awaitLaneRoom() {
      ...
      while (!this._stopRequested && this._lane.length >= cap) {
        if (!told) {
          told = true;
          log(`Generation paused — ${this.laneImages()} image(s) still waiting on the ` + `inspector. Raise "QC lane depth" only if you want a deeper backlog.`);
        }
    ...
    setPhase(text) {
      ...
      State.setWorker({
        statusText: this._phaseText() || (text || 'Idle')
      });
    },
    setLanePhase(text) {
      this._lanePhase = text ? {
        text: text,
        at: Date.now()
      } : null;
      State.setWorker({
        statusText: this._phaseText() || 'Idle',
        qcLane: this.laneImages()
      });
    },
    _phaseText() {
      const stamp = p => {
        if (!p) return null;
        const sec = Math.round((Date.now() - p.at) / 1e3);
    ...
    async processJob(job) {
      ...
      try {
        ...
        if (Array.isArray(job.referenceIds) && job.referenceIds.length) {
          if (generationDriver === this._comfyDriver) {
            ...
            job.referenceFetchErrors = job.referenceIds.map(id => ({
              id: String(id),
              error: 'the active generation engine does not accept image references'
            }));
        ...
        const result = await generationDriver.generate(job.prompt, {
          filters: job.controls,
          references: references,
          shouldStop: () => this._stopRequested,
          count: Number(job.count) > 0 ? Number(job.count) : 0
        }, log);
        const refResult = result.referenceResult || null;
        job.attachedReferenceIds = refResult ? [ ...refResult.attached || [] ] : [];
        job.referenceErrors = [ ...Array.isArray(job.referenceFetchErrors) ? job.referenceFetchErrors : [], ...refResult && Array.isArray(refResult.failed) ? refResult.failed : [], ...refResult && Array.isArray(refResult.skipped) ? refResult.skipped : [] ].slice(0, 8).map(e => ({
          id: String(e.id || ''),
        ...
        for (const img of images) {
          items.push({
            card: await this.saveCard(job, img),
            base64: img.base64,
            mime: img.mime
          });
        }
    ...
    enqueueBatch(batch) {
      ...
      if (!this._laneRunning) this._lanePromise = this.runLane(); else State.setWorker({
        qcLane: this.laneImages()
      });
    },
    async runLane() {
    ...
    async finishBatch(batch) {
      const {job: job, items: items} = batch;
      ...
      try {
        ...
        phase(manual ? `Finishing ${total} image(s)…` : `Inspecting ${total} image(s)${lead ? ` via ${lead.name}` : ''}…`);
        log(manual ? `AI quality check is off — ${total} image(s) go straight to Review for manual sorting…` : `Inspecting ${total} image(s)${lead ? ` via ${lead.name}` : ''}` + `${limit > 1 ? ` — ${limit} at a time` : ' — one at a time'}…`);
        const cards = await U.mapLimit(items, limit, async it => this.inspectSaved(job, it, {
          skipInspection: this._parkInsteadOfInspect()
        }));
        const passed = cards.filter(c => c && c.status === 'metadata');
        const stalled = cards.filter(c => c && c.status === 'qc_error').length;
        log(manual ? `${passed.length} image(s) saved in ${Math.round((Date.now() - qcStart) / 1e3)}s — none inspected, all go to Review.` : `QC done in ${Math.round((Date.now() - qcStart) / 1e3)}s — ${passed.length} passed` + `, ${cards.filter(c => c && c.status === 'discarded').length} failed` + (stalled ? `, ${stalled} could not be inspected (kept for retry)` : ''));
        const blind = passed.length && !metadataOff() && metadataNeedsQc() ? passed.filter(c => c && c.qcSkipped) : [];
        const described = blind.length ? passed.filter(c => !(c && c.qcSkipped)) : passed;
        if (blind.length) {
          log(`${blind.length} image(s) were not inspected — they go to Review bare rather than ` + `carrying a title written from the prompt alone. Turn the AI quality check on, or ` + `untick "only for inspected images" in Settings → Automatic metadata.`);
          this.promoteWithoutMetadata(blind);
        }
        if (passed.length && metadataOff()) {
          phase(`${passed.length} image(s) → Review…`);
          log(`${passed.length}/${total} ready — auto metadata is off, so they go to Review bare. ` + `Use "Write metadata" on the ones you like.`);
        ...
        if (passCount === 0 && stalled === total) {
          ...
          log(`Job parked — none of ${total} image(s) were inspected ` + `(${this._stopRequested ? 'worker stopped' : 'vision model unreachable'}). ` + `They are kept, not discarded.`, 'err');
        } else if (passCount === 0) {
          job.attempts += 1;
          const max = State.settings.gen.maxRetries || 0;
          if (job.attempts <= max) {
    ...
    async inspect(base64, mime, prompt) {
      ...
      const metrics = State.settings.gen.qcMetrics !== false ? await this.qcMetrics(small.base64, small.mime).catch(() => null) : null;
      const r = await U.llmVision(small.base64, small.mime, T.qc(prompt, threshold), {}, 'Vision QC');
      let qc;
      try {
        qc = validateQc(U.extractJson(r.text));
      } catch (e) {
        e.qcAttempt = {
          stage: 'inspection',
      ...
      const worth = State.settings.gen.qcGeneralPass !== false && this.applyQcResult({}, qc, meta);
      ...
      if (worth || confirm) {
        ...
        try {
          const g = await U.llmVision(small.base64, small.mime, T.qcGates(), {}, worth ? 'Vision QC (general)' : 'Vision QC (checking a veto)');
          ...
          log(worth ? `General QC pass could not run — image kept for retry (${e.message}).` : `Second look at a vetoed image could not run — image kept for retry (${e.message}).`, 'err');
          e.qcAttempt = {
            stage: worth ? 'general' : 'confirm-veto',
            inspection: qc,
            engine: meta.engine || 'unknown',
            model: meta.model || '',
            gateEngine: meta.gateEngine || 'unknown',
            gateModel: meta.gateModel || '',
            gates: meta.gates || null,
            promptVersion: QC_PROMPT_VERSION,
            policyVersion: QC_POLICY_VERSION,
            error: e.message
          };
    ...
    async downscaleForQc(base64, mime) {
      ...
      try {
        ...
        if (!longest || longest <= maxEdge) return {
          base64: base64,
          mime: mime
        };
    ...
    async qcMetrics(base64, mime) {
      ...
      for (let i = 0, p = 0; i < lum.length; i++, p += 4) lum[i] = .299 * px[p] + .587 * px[p + 1] + .114 * px[p + 2];
      let mean = 0;
    ...
    qcVetoes(defects, fingerCounts, regions = null) {
      ...
      const hands = (Array.isArray(regions) ? regions : []).find(r => r && typeof r === 'object' && /hand/i.test(String(r.region || '')));
      const fr = fingerReport(fingerCounts, {
        handsVerdict: hands ? hands.verdict : null
      });
      return {
        words: words,
        fingers: fr.bad.map(h => h.said),
        hands: fr.hands
      };
    },
    ...
    async saveCard(job, img) {
      ...
      const card = {
        ...
        theme: job.theme,
        prompt: job.prompt,
        promptSource: job.promptSource || 'ideation',
        learnedFrom: job.learnedFrom || null,
        ...
        chatReferences: Array.isArray(job.chatReferences) ? job.chatReferences.slice(0, 16).map(r => ({
          id: String(r.id || ''),
          fname: String(r.fname || ''),
          name: String(r.name || '').slice(0, 80)
        })) : [],
        attachedReferenceIds: Array.isArray(job.attachedReferenceIds) ? [ ...job.attachedReferenceIds ] : [],
        referenceErrors: Array.isArray(job.referenceErrors) ? job.referenceErrors.slice(0, 8).map(e => ({
          id: String(e.id || ''),
          error: String(e.error || '').slice(0, 300)
        })) : [],
    ...
    async inspectSaved(job, item, {skipInspection: skipInspection = false} = {}) {
    ...
    qcCalibration(n = 40) {
      const threshold = State.settings.gen.passThreshold || 7;
      const scored = (State.library || []).filter(c => c && c.qc && typeof c.qc.score === 'number').slice(0, n);
      ...
      for (const c of scored) {
        ...
        if (by.includes('veto')) blockers.veto++; else if (by.includes('general')) blockers.general++; else if (by.includes('detail')) blockers.detail++; else if (c.qc.score < threshold) blockers.score++; else blockers.inspector++;
      }
      return {
        n: scores.length,
        threshold: threshold,
        passRate: scored.filter(passed).length / scored.length,
        suggest: suggest,
        suggestRate: rateAt(suggest),
        blockers: blockers
      };
    },
    qcCalibrationNote(cal) {
      if (!cal || cal.passRate >= .15) return null;
      ...
      switch (top[0]) {
       case 'veto':
        return `${head} — the hand/anatomy veto failed ${top[1]} of the ${failed}. The inspector's own words did it, so a lower pass bar cannot help;` + ' Settings → Quality check → "Check a veto with a second look" lets a disputed frame through, and "Believe what the inspector describes" switches the veto off.';
       ...
       default:
        return cal.suggest < cal.threshold ? `${head} at a bar of ${cal.threshold} — this inspector scores lower than that. ${cal.suggest} would pass ${Math.round(cal.suggestRate * 100)}%. Settings → Generation → Pass threshold.` : `${head}; ${top[1]} of the ${failed} scored under the bar of ${cal.threshold}.`;
      }
    },
    noteQcOutcome() {
      const w = this._qcWatch || (this._qcWatch = {
        n: 0,
        saidAt: 0
      });
      w.n += 1;
      if (w.n % 20 !== 0 || Date.now() - w.saidAt < 30 * 6e4) return null;
    ...
    applyQcResult(card, qc, qcMeta = {}) {
      ...
      const defects = (Array.isArray(qc.defects) ? qc.defects : []).map(d => d && typeof d === 'object' ? {
        what: String(d.what || d.defect || ''),
        where: String(d.where || ''),
        severity: String(d.severity || 'noticeable').trim().toLowerCase(),
        area: String(d.area || ''),
      ...
      const veto = vetoOn ? this.qcVetoes(defects, qc.fingerCounts, qc.regions) : {
        words: [],
        fingers: [],
        hands: []
      };
      ...
      const caps = [];
      if (vetoed) {
        const said = [ veto.words.length ? `its own defect list says "${veto.words.join('", "')}"` : '', veto.fingers.length ? `it counted the fingers as ${veto.fingers.join(' / ')}` : '' ].filter(Boolean).join('; ');
        if (disputed) {
          ...
          caps.push({
            by: 'veto',
            to: SEVERE_BAND,
            why: checkedVeto ? `${said}; a second look agreed (${gates.failed.filter(k => STRUCTURAL_GATES.includes(k)).join(', ')})` : said
          });
          score = SEVERE_BAND;
        }
      }
      if (gates && gates.ceiling < score) {
        caps.push({
          by: 'general',
          to: gates.ceiling,
          why: gates.no ? `${gates.no} of ${gates.answered} general gates answered NO (${gates.failed.join(', ')})${gates.worst && !/^nothing/i.test(gates.worst) ? ` — ${gates.worst}` : ''}` : `said this is a ${gates.overall}/10 on its own`
        });
        score = gates.ceiling;
      }
      ...
      card.qc = {
        ...
        veto: {
          words: veto.words,
          fingers: veto.fingers,
          ...checkedVeto ? {
            checked: disputed ? 'disputed' : 'confirmed'
          } : {}
        },
        defects: defects.map(d => d.where ? `${d.what} (${d.where})` : d.what).filter(Boolean),
        detail: defects,
        regions: (Array.isArray(qc.regions) ? qc.regions : []).map(r => r && typeof r === 'object' ? {
    ...
    async retryQc(card) {
      ...
      if (applied) {
        ...
        card.status = 'review';
        if (fromQcError) State.bumpStats({
          imagesPassed: 1
        });
      } else if (fromQcError) {
        card.status = 'discarded';
    ...
    async applyBatchMetadata(job, cards) {
      ...
      const variety = Titles.promptBlock({
        prompt: job.prompt,
        count: cards.length,
        devices: devices
      });
      ...
      while (list && budget > 0) {
        ...
        budget--;
        log(`${problems.length} title(s) repeated something already used — asking for replacements: ` + problems.map(p => `"${p.title}" (${p.reason})`).join('; '), 'err');
        const fixed = await this._askReplacementTitles(job, problems);
        if (!fixed) break;
        for (let i = 0; i < problems.length; i++) {
          const candidate = String(fixed[i] || '').trim();
          if (!candidate) continue;
          const before = Titles.similarity(problems[i].title, (Titles.nearest(problems[i].title, {
            exclude: [ problems[i].title ]
          }) || {}).t || '');
          const after = Titles.isRepeat(candidate, {
            exclude: [ candidate ]
          });
      ...
      for (let i = 0; i < cards.length; i++) {
        ...
        card.metadata = entry ? this.shapeMetadata(entry, destinationsOf(card)) : await this.genMetadata(job.prompt, card.qc, destinationsOf(card)).catch(() => null);
      ...
      try {
        const {text: text} = await U.llmChat([ {
          ...
          content: T.retitle({
            prompt: job.prompt,
            slots: problems,
            variety: Titles.promptBlock({
              prompt: job.prompt,
              count: problems.length
            })
          })
        } ], {
          temperature: Titles.temperature(.15),
          maxTokens: 4e3 + 400 * problems.length,
          role: 'metadata'
        }, 'Replacement titles');
    ...
    async _askBatchMetadata(job, cards, variety) {
      ...
      const titleHints = window.Insights && window.Insights.playbook ? (window.Insights.playbook.titleTraits || []).filter(t => t.lift >= 1.2).slice(0, 3).map(t => t.key) : [];
      try {
        const {text: text, provider: provider, tier: tier, model: model} = await U.llmChat([ {
          role: 'user',
          content: T.metadataBatch({
            prompt: job.prompt,
            count: cards.length,
          ...
          maxTokens: 1200 * cards.length + 4e3,
          role: 'metadata'
        }, 'Batch metadata');
        this._lastMetaEngine = {
          provider: provider,
          tier: tier,
          model: model,
          at: Date.now()
    ...
    shapeMetadata(raw, destination = 'deviantart') {
      ...
      return {
        ...
        titleAlts: (Array.isArray(raw.alts) ? raw.alts : []).map(t => String(t || '').replace(/["#]/g, '').trim().slice(0, 50)).filter(t => t && t.toLowerCase() !== written.toLowerCase()).slice(0, 4),
    ...
    async genMetadata(prompt, qc, destination = 'deviantart') {
      ...
      const {text: text, provider: provider, tier: tier, model: model} = await U.llmChat([ {
        ...
        temperature: Titles.temperature(),
        maxTokens: 3500 + descriptionHeadroom(),
        role: 'metadata'
      }, 'Metadata');
      this._lastMetaEngine = {
        provider: provider,
    ...
    async retitleCard(card, reason = '') {
      ...
      const verdict = current ? Titles.isRepeat(current, {
        exclude: [ current ]
      }) : {
        repeat: false
      };
      const slot = {
        title: current || '(none)',
        reason: reason || (verdict.repeat ? `too close to "${verdict.against.t}"` : 'the artist asked for a different one')
      };
      const {text: text, provider: provider, tier: tier, model: model} = await U.llmChat([ {
        role: 'user',
        content: T.retitle({
      ...
      const next = String(list && (typeof list[0] === 'string' ? list[0] : (list[0] || {}).title) || '').replace(/["#]/g, '').trim().slice(0, 50);
    ...
    async enhanceCardMetadata(card) {
      ...
      try {
        ...
        const r = await U.llmVision(small.base64, small.mime, T.metadataVision({
          ...
          temperature: Titles.temperature(),
          maxTokens: 6e3 + descriptionHeadroom()
        }, 'Metadata from image');
    ...
    metadataNeedsQc: metadataNeedsQc,
    ageDisclaimer: ageDisclaimer,
    hasAgeDisclaimer: hasAgeDisclaimer,
    withAgeDisclaimer: withAgeDisclaimer,
    publishField: publishField,
    descriptionBrief: descriptionBrief,
    descriptionHeadroom: descriptionHeadroom,
    DESCRIPTION_BRIEFS: DESCRIPTION_BRIEFS,
    destinationOf: destinationOf,
    destinationsOf: destinationsOf,
    goesTo: goesTo,
    DESTINATIONS: DESTINATIONS,
    stripPatreonBlock: stripPatreonBlock,
    addPatreonBlock: addPatreonBlock,
    wantsPatreonBlock: wantsPatreonBlock,
    setDestinations(card, list) {
      ...
      card.destination = next.includes('patreon') ? 'patreon' : next.includes('deviantart') ? 'deviantart' : next[0];
      if (wantedBlock !== wantsBlock && card.metadata && typeof card.metadata.description === 'string') {
        card.metadata.description = wantsBlock ? addPatreonBlock(card.metadata.description, p) : stripPatreonBlock(card.metadata.description, p);
      }
      card.updatedAt = Date.now();
      return true;
    },
    ...
    toggleDestination(card, id) {
      ...
      const next = before.includes('patreon') ? [ id ] : before.includes(id) ? before.filter(d => d !== id) : [ ...before, id ];
      if (!next.length) return false;
    ...
    async uploadCard(card) {
      ...
      let res;
      try {
        res = method === 'api' ? await window.ala.da.stashDraft({
          filePath: card.path,
    ...
    applyUploadResult(card, res, method = 'session', elapsedMs = 0) {
      if (res.ok) {
        ...
        if (res.published) this.markPublished(card, res.url); else if (res.publishError) {
          card.da.publishError = {
            message: res.publishError,
            kind: res.publishKind || 'permanent',
            at: Date.now()
          };
        }
        try {
          if (window.Origins) Origins.record(card, {
            link: 'upload'
          });
        } catch {}
        card.updatedAt = Date.now();
        ...
        log(`${how} (${method}) in ${Math.round(elapsedMs / 1e3)}s: ${card.metadata.title}`, 'ok');
    ...
    async publishCard(card) {
      ...
      if (card.da.published) return {
        ok: true,
        url: card.da.url,
        alreadyPublished: true
      };
      const res = await window.ala.daweb.publish(card.da.deviationId).catch(e => ({
        ok: false,
        error: e.message,
        kind: 'transient'
      }));
      if (res.ok) {
        ...
        card.da.publishError = {
          message: res.error || 'submit failed',
          kind: res.kind || 'permanent',
          at: Date.now()
        };
    ...
    async retryDraftMetadata(card) {
      ...
      if (res.ok) {
        card.da = {
          ...card.da,
          deviationId: res.deviationId || card.da.deviationId,
          partial: false
        };
        card.error = null;
        card.uploadError = null;
    ...
    async sendEverywhere(card, {onSite: onSite, skip: skip = []} = {}) {
      const sites = pendingSites(card).filter(s => !skip.includes(s));
      const results = {};
      for (const site of sites) {
        if (onSite) onSite(site);
        try {
          if (site === 'deviantart') results.deviantart = await this.uploadCard(card); else if (site === 'pixiv') results.pixiv = await this.postCardToPixiv(card);
        } catch (e) {
          results[site] = {
            ok: false,
            error: e.message,
    ...
    async postCardToPixiv(card) {
      ...
      if (card.pixiv && card.pixiv.illustId) return {
        ok: true,
        alreadyPosted: true,
        illustId: card.pixiv.illustId
      };
    ...
    applyPixivResult(card, res, elapsedMs = 0) {
      ...
      if (res && res.ok) {
        ...
        log(`Posted to pixiv in ${Math.round(elapsedMs / 1e3)}s: ${card.metadata.title}`, 'ok');
        return card;
      }
      ...
      if (kind === 'pending') {
        ...
        if (card.pixivAttempts < max) {
          card.nextPixivRetryAt = Date.now() + RETRY_BACKOFF_MS[Math.min(card.pixivAttempts - 1, RETRY_BACKOFF_MS.length - 1)];
    ...
    async prepareCardOnPixiv(card) {
      ...
      try {
        const payload = this.pixivPayload(card);
        const base64 = await window.ala.files.readImageBase64(card.fname);
        res = await window.Pixiv.prepare({
          ...payload,
          base64: base64
        });
      } catch (e) {
      ...
      card.pixivError = null;
      State.persistLibrary();
      log(`Staged "${card.metadata.title}" on the pixiv upload form — press Post on the Pixiv tab.` + (res.missed?.length ? ` Not filled: ${res.missed.join(', ')}.` : ''), res.critical?.length ? 'err' : 'ok');
      return {
        ok: true,
        ...res
      };
    ...
    async harvestPixiv(card) {
      ...
      this.applyPixivResult(card, {
        ...
        url: `https://www.pixiv.net/artworks/${res.illustId}`
      }, Date.now() - prep.at);
      return {
        ok: true,
        found: true,
  ...
  const AutoMode = {
    ...
    stopCondition() {
      ...
      if (hours > 0 && this.elapsedMs() >= hours * 36e5) {
      ...
      if (down && Date.now() - down.since >= 30 * 6e4) {
        return `the image engine has not answered for ${Math.round((Date.now() - down.since) / 6e4)} minutes (${down.error})`;
      }
    ...
    noteRoundOutcome() {
      ...
      if (jobs.length && jobs.every(j => j.status === 'failed')) {
        ...
        log(`Every job from the last round failed (${this._lastJobError || 'no error recorded'}) — ` + `${this._roundsAllFailed} such round(s) in a row; auto mode stops at 3.`, 'err');
      } else {
    ...
    chooseTheme() {
      ...
      if (!rr) return null;
      const themes = (this.settings.themes || []).filter(t => t && String(t.theme || '').trim());
      const learningOn = this.settings.useLearning !== false && State.settings.learn && State.settings.learn.enabled !== false && window.Insights;
      if (!learningOn || themes.length < 2) return rr;
      const EXPLORE = 1 / 3;
      if (Math.random() < EXPLORE) return {
        ...rr,
        _pick: 'rotation'
      };
      ...
      const scored = themes.map((t, i) => ({
        t: t,
        i: i,
        w: weights.get(String(t.theme).toLowerCase().replace(/[^a-z0-9]+/g, ' ').trim()) || 1
      }));
    ...
    throttleDelayMs() {
      ...
      return Math.max(0, this._imageTimestamps[0] + 36e5 - Date.now()) + 1e3;
    },
    noteImages(n) {
      const now = Date.now();
    ...
    async start() {
      ...
      try {
        ...
        log(`Auto mode stopped${this.stopReason ? ` — ${this.stopReason}` : ''}. ` + `${this.settings.roundsDone} round(s), ${this.reviewBacklog()} card(s) waiting in Review.`, 'ok');
        osNotify('AiLabor — auto mode stopped', `${this.stopReason || 'stopped'} · ${this.settings.roundsDone} round(s), ` + `${this.reviewBacklog()} card(s) waiting in Review.`);
      }
    },
    stop(reason) {
      if (!this.running) return;
    ...
    async gpuContention() {
      const now = Date.now();
      if (this._contention && now - this._contention.at < 6e4) return this._contention.v;
      let v = true;
      try {
        const g = State.settings.gen || {};
        const laneRoles = [];
        ...
        if (!laneRoles.length) {
          ...
          const [writer, lane] = await Promise.all([ Promise.all([ 'ideation', 'metadata' ].map(isLocal)), Promise.all(laneRoles.map(isLocal)) ]);
    ...
    async loop() {
      while (!this._stop) {
        ...
        if (pending > 0 || State.worker.currentJobId) {
          ...
          const down = Pipeline.engineDown;
          this.setStatus(down ? `Waiting — the image engine is not answering (${Math.floor((Date.now() - down.since) / 6e4)} min; stops at 30)` : pending ? `Waiting — ${pending} job(s) still in the queue` : 'Waiting — the worker is finishing a job');
          await U.sleep(1e4);
          continue;
        }
        ...
        if (wait > 0) {
          this.setStatus(`Throttled — ${Math.ceil(wait / 6e4)}m until the hourly image cap resets`);
          await U.sleep(Math.min(wait, 6e4));
          continue;
        }
        ...
        if (!this._stop) await U.sleep(3e3);
      }
    },
    async maybeRelearn(round) {
      ...
      try {
        const p = await window.Insights.refresh({
          log: log
        });
    ...
    async runRound() {
      ...
      if (!theme) {
        this.stopReason = 'no themes configured';
        this._stop = true;
        return false;
      }
      const s = this.settings;
      ...
      const ratio = Math.max(0, Math.min(1, Number(s.exploitRatio ?? .4)));
      const wantEvolved = s.useLearning === false || mode !== 'exploit' ? 0 : Math.min(count - 1, Math.round(count * ratio));
      ...
      if (wantEvolved > 0 && window.PromptLab) {
        ...
        try {
          const grown = await PromptLab.fromWinners({
            count: wantEvolved,
            theme: theme.theme,
            avoid: avoid,
            axes: axes,
      ...
      let queued = prompts;
      if (window.Teach) {
        const {kept: kept, dropped: dropped} = Teach.screen(prompts.map(p => p.prompt));
        if (dropped.length) {
      ...
      log(`Auto round ${round}: queued ${jobs.length} prompt(s) for "${theme.theme}"` + `${evolved ? ` (${evolved} grown from a top performer)` : ''}` + `${V ? ` · ${V.describeMode(mode)}` : ''}` + `${axes && axes.length ? ` · varying ${axes.map(a => a.axis).join('/')}` : ''}` + `${theme._pick ? ` · theme picked by ${theme._pick}` : ''}.`, 'ok');
