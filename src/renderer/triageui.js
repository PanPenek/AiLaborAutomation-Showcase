(function() {
  ...
  const prefs = () => ({
    ...PREFS_DEFAULT,
    ...State.settings.ui && State.settings.ui.triage || {}
  });
  ...
  function syncSelection() {
    ...
    if (selFor && touched) remembered.set(selFor, {
      ids: [ ...sel ],
      source: selSource
    });
  ...
  function openTriage(opts = {}) {
    ...
    open = true;
    if (!session || Date.now() - session.last > 30 * 6e4) {
      session = {
        started: Date.now(),
        last: Date.now(),
        batches: 0,
  ...
  function tileHtml(c, i) {
    ...
    const pk = c.pick;
    const qc = c.qc && typeof c.qc.score === 'number' ? `<span class="tri-qc ${c.qc.verdict === 'PASS' ? 'ok' : 'bad'}">QC ${c.qc.score}</span>` : '';
    const mark = pk ? pk.best ? '✨ best · ' : pk.keep ? '✨ keep · ' : pk.verdict === 'broken' ? '⚠ ' : '' : '';
    const ai = pk ? `<div class="tri-ai-note ${esc(pk.verdict)}" title="${esc(`${pk.verdict}${pk.issue ? ': ' + pk.issue : ''}${pk.engine ? ` — ${pk.engine}${pk.model ? ' · ' + pk.model : ''}` : ''}`)}">${mark}${esc(pk.issue || pk.verdict)}</div>` : '';
  ...
  function bodyHtml(b) {
    ...
    const middle = zoom >= 0 && b.cards[zoom] ? zoomHtml(b, zoom) : `<div class="tri-grid" style="grid-template-columns:repeat(${cols}, 1fr)">${b.cards.map(tileHtml).join('')}</div>`;
  ...
  function signature(b) {
    return JSON.stringify([ b && b.key, b && b.cards.map(c => [ c.id, c.pick && c.pick.at, Triage.isKept(c) ]), zoom, zoom >= 0 ? [ ...sel ] : null, b && busy.has(b.key), undoStack.length, prefs(), window.innerWidth, window.innerHeight ]);
  }
  function render() {
    if (!open) return;
    syncSelection();
    ...
    if (sig !== bodySig) {
      ...
      body.querySelectorAll('.tri-tile').forEach(el => {
        ...
        el.addEventListener('dblclick', () => {
          zoom = Number(el.dataset.i);
          render();
        });
      });
      body.querySelectorAll('[data-zoom]').forEach(z => z.addEventListener('click', () => {
  ...
  function commit(keepIds) {
    ...
    const rec = Triage.commit(b, keepIds, {
      keepTo: prefs().keepTo,
      aiSource: touched ? 'edited' : selSource
    });
    ...
    undoStack.push({
      rec: rec,
      kept: kept.length,
      discarded: b.cards.length - kept.length
    });
  ...
  function writeTitles(cards) {
    ...
    metaChain = metaChain.then(async () => {
      try {
        const res = mode === 'image' ? await Pipeline.enhanceMetadataForCards(need) : await Pipeline.writeMetadataForCards(need);
  ...
  async function askAi(b = current(), {quiet: quiet = false} = {}) {
    ...
    try {
      ...
      if (!quiet) toast('The AI could not pick: ' + e.message, 'err'); else State.addLog(`AI pre-pick skipped a batch: ${e.message}`, 'err');
    } finally {
      busy.delete(b.key);
  ...
  async function pumpAhead() {
    ...
    try {
      const tried = new Set;
      while (open && aiOn()) {
        const next = list.slice(idx, idx + AHEAD).find(b => b.cards.length > 1 && !b.cards.some(c => c.pick) && !busy.has(b.key) && !tried.has(b.key));
        if (!next) break;
        tried.add(next.key);
        await askAi(next, {
          quiet: true
  ...
  function act(what) {
    ...
    switch (what) {
     ...
     case 'commit':
      if (!b) return;
      if (!sel.size) {
        toast('Nothing picked yet — press a tile’s number to keep it, or X to discard the whole batch.', 'err');
        return;
      }
      commit([ ...sel ]);
      return;
  ...
  function onKey(ev) {
    ...
    if (ev.repeat && (k === 'Enter' || k === 'x' || k === 'X' || k === 'Delete')) {
      handled();
      return;
    }
    if (zoom >= 0) {
  ...
  window.TriageUI = {
    ...
    count(st = 'review') {
      const b = Triage.batches({
        status: st
      });
      return {
        batches: b.length,
        cards: b.reduce((n, x) => n + x.cards.length, 0)
      };
    }
  };
})();
