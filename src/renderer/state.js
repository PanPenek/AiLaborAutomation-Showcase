(function() {
  const listeners = {};
  function looksLikePrompt(s, {min: min = 40, max: max = 1200} = {}) {
        ...
        if (c === '"') inStr = true; else if (c === open) depth++; else if (c === close) {
          ...
          if (depth === 0) {
            try {
              out.push({
                at: start,
                value: JSON.parse(cleaned.slice(start, i + 1))
              });
            } catch {}
            break;
          }
  ...
  function repairedCandidates(text, {limit: limit = 8, tries: tries = 60} = {}) {
    const cleaned = String(text || '').replace(/```(?:json)?/gi, '');
    const out = [];
    let attempts = 0;
  ...
  function salvagePrompts(text, {min: min = 40, max: max = 1200, limit: limit = 16} = {}) {
    const quoted = [ ...String(text || '').matchAll(QUOTED_RUN) ].map(m => unescapeJsonish(m[1]).trim()).filter(s => s.length >= min && s.length <= max);
    const out = [];
  ...
  const MEDIUM_LEAD_RE = new RegExp(String.raw`^\s*(?:an?|the)?\s*(?:` + String.raw`(?:${MEDIUM_QUALIFIER}\s+)+(?:${MEDIUM_NOUN}|art)(?:\s*${MEDIUM_OF}\s+|\s*[,:;.\-\u2013\u2014]\s*|\s+)` + String.raw`|${MEDIUM_NOUN}\s+${MEDIUM_OF}\s+` + String.raw`)`, 'i');
  ...
  function housePrompt(text) {
    ...
    return s.replace(/\s+/g, ' ').replace(/\s+,/g, ',').replace(/,(\s*,)+/g, ',').replace(/^[,\s]+/, '').replace(/[,;\s]+$/, '').trim();
  }
  function promptsFrom(res, {count: count = 8, min: min = 40, max: max = 900} = {}) {
    ...
    const clean = v => {
      ...
      if (!list) return [];
      return list.map(s => housePrompt(s)).filter(s => looksLikePrompt(s, {
        min: min,
        max: max
      }));
    };
    const scored = jsonCandidates(text).map(c => ({
      at: c.at,
      list: clean(c.value)
    ...
    const mended = repairedCandidates(text).map(c => ({
      at: c.at,
      list: clean(c.value)
    })).filter(c => c.list.length);
    if (mended.length) {
    ...
    return thinking ? [] : salvagePrompts(text, {
      min: min,
      max: max,
      limit: count
    });
  }
  function ideasFrom(res, {count: count = 8, min: min = 40, max: max = 900} = {}) {
    const text = typeof res === 'string' ? res : String(res && res.text || '');
    const clean = v => {
      const list = Array.isArray(v) ? v : v && Array.isArray(v.prompts) ? v.prompts : null;
      if (!list) return [];
  ...
  window.U = {
    ...
    unescapeJsonish: unescapeJsonish,
    ideasFrom: ideasFrom,
    extractJson(text) {
    ...
    looksLikePrompt: looksLikePrompt,
    salvagePrompts: salvagePrompts,
    promptsFrom: promptsFrom,
    housePrompt: housePrompt,
    MEDIUM_LEAD_RE: MEDIUM_LEAD_RE,
  ...
  async function record(role, label, run) {
    ...
    try {
      ...
      if (notes.length) {
        State.addLog(`${label} was answered by ${r.provider || r.engine} ` + `after ${notes.join(', ')} — check Settings → Providers if that keeps happening.`, 'err');
      }
  ...
  const persistQueueDebounced = U.debounce(items => window.ala.db.setQueue({
    items: items
  }), 500);
  const persistLibraryDebounced = U.debounce(items => window.ala.db.setLibrary({
    items: items
  }), 500);
  window.State = {
    settings: null,
    ...
    async init() {
      ...
      for (const c of this.library) {
        ...
        if (c.status === 'qc') {
          c.status = 'qc_error';
          c.error = c.error || 'QC interrupted — app restarted';
        }
    ...
    persistQueue() {
      persistQueueDebounced(this.queue);
