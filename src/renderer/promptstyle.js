(function() {
  ...
  function trim(text, max) {
    ...
    return (at > max * .5 ? cut.slice(0, at) : cut.trimEnd()) + '…';
  }
  function houseScore(p, prof = null) {
    ...
    if (prof && prof.enough) {
      ...
      if (w >= prof.lo && w <= prof.hi) score += 1; else if (w > prof.hi * 1.5 || w < prof.lo * .6) score -= 2;
    }
    return score;
  }
  function corpus({limit: limit = 60} = {}) {
    const out = [];
    const seen = new Set;
    const push = (text, src) => {
    ...
    const automaticIds = new Set([ ...entries, ...devs, ...lib ].filter(r => automatic.has(r.promptSource)).map(r => String(r.deviationId || r.da && r.da.deviationId || '')).filter(Boolean));
  ...
  function profile(list = null) {
    ...
    if (rows.length < MIN_CORPUS) return {
      n: rows.length,
      enough: false
    };
    ...
    const tail = [ ...tally.entries() ].filter(([, n]) => n >= Math.max(2, Math.round(rows.length * .1))).sort((a, b) => b[1] - a[1]).slice(0, 10).map(([k]) => k);
    return {
      n: rows.length,
      enough: true,
      words: median(w),
      lo: Math.max(30, Math.round(median(w) * .7)),
  ...
  function exemplars({n: n = SHOW, from: from = null} = {}) {
    const rows = (from || corpus()).map(c => typeof c === 'string' ? {
      text: c,
      src: ''
    } : c);
    const ranked = rows.map(r => ({
      ...r,
      score: houseScore(r.text)
    })).filter(r => r.score > 0).sort((a, b) => b.score - a.score);
    const sim = window.promptSimilarity || (() => 0);
    const kept = [];
    for (const r of ranked) {
      if (kept.length >= n) break;
      if (!kept.some(k => sim(k.text, r.text) > .5)) kept.push(r);
    }
    return kept;
  }
  function block({n: n = SHOW, maxChars: maxChars = 2200} = {}) {
    const rows = corpus();
    ...
    const lines = [ `HOW THIS ARTIST WRITES PROMPTS — measured on ${p.n} of his own, not invented. Match this form:`, '- Comma-separated fragments, not sentences. No narration and no scene-setting prose:' + ' write what is in the picture, not what is happening to the person in it.', '- NEVER open by naming the medium. "anime illustration", "digital art of", "an image of",' + ' "a portrait of" are banned openings. The first fragment is the subject and what is' + ' being done to it — that is the position the generator weights hardest, and it is not' + ' for the word "illustration".', `- About ${p.words} words across ${p.frags} comma-separated fragments (${p.lo}-${p.hi} words is the band).` ];
    if (p.lowerStart >= .6) {
    ...
    lines.push('', 'HIS OWN PROMPTS — copy the shape, not the content:', ...ex.map((r, i) => `[${i + 1}] """${trim(r.text, 700)}"""`));
