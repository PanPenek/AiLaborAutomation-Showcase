(function() {
  ...
  const cancelled = () => ({
    ok: false,
    cancelled: true,
    error: 'Stopped by the artist; no further actions were started.'
  });
  function resultText(value) {
    if (value == null) value = {
      ok: false,
      error: 'The tool returned no result; completion is unconfirmed.'
    };
    try {
      ...
      const summary = {
        truncated: true,
        note: 'Result shortened; use the saved id for the next action instead of copying the omitted payload.'
      };
      for (const key of [ 'ok', 'error', 'refused', 'cancelled', 'queued', 'written', 'failed', 'matched', 'idle', 'researchId', 'promptSetId', 'sourceCount', 'imageCount', 'referenceCount', 'referenceCeiling', 'skipped', 'cardId', 'video', 'where', 'lengthSeconds', 'source', 'selfCheck' ]) {
      ...
      if (Array.isArray(value.images)) summary.images = value.images.slice(0, 8).map(im => ({
        id: im.id,
        title: String(im.title || '').slice(0, 160),
      ...
      const shrink = key => {
        while (Array.isArray(summary[key]) && summary[key].length && JSON.stringify(summary).length > TOOL_RESULT_CAP) summary[key].pop();
      };
      shrink('prompts');
      shrink('images');
      shrink('sources');
      ...
      if (JSON.stringify(summary).length <= TOOL_RESULT_CAP && (value.researchId || value.promptSetId || value.sources || value.images || value.prompts)) {
  ...
  const TOOLS = {
    ...
    read_stats: {
      ...
      run: ({top: top = 5} = {}) => {
        if (!window.Insights) return {
          error: 'the stats module is not loaded'
        };
        const ov = Insights.overview();
        const ranked = Insights.topPerformers ? Insights.topPerformers(clamp(top, 1, 20)) : Insights.scored().filter(({m: m}) => m.perDay != null).sort((a, b) => b.m.perDay - a.m.perDay).slice(0, clamp(top, 1, 20));
        const scored = ranked.map(({d: d, m: m}) => ({
          title: d.title,
          theme: d.theme || '',
    ...
    read_playbook: {
      ...
      run: ({theme: theme = ''} = {}) => {
        ...
        const text = Insights.guidance({
          theme: theme,
          maxChars: 2e3
        });
        return {
          guidance: text || '(nothing measured yet — there is no playbook to read)'
        };
      }
    },
    web_research: {
    ...
    list_cards: {
      ...
      run: ({filter: filter = 'agent', limit: limit = 20} = {}) => {
    ...
    learn: {
      ...
      run: async () => {
        ...
        const p = await Insights.refresh({
          log: log
        }).catch(e => ({
          error: e.message
        }));
    ...
    worker: {
      ...
      run: ({action: action} = {}) => {
        if (![ 'start', 'stop' ].includes(action)) return {
          ok: false,
          error: 'worker action must be start or stop'
        };
        if (action === 'stop') {
          Pipeline.stop();
          return {
        ...
        return Pipeline.running ? {
          ...
          ok: false,
          running: false,
          error: 'The worker did not start; check the generation driver.'
        };
      }
    },
    auto_mode: {
      ...
      run: ({action: action} = {}) => {
        if (!window.AutoMode) return {
          error: 'auto mode is not loaded'
        };
        if (![ 'start', 'stop' ].includes(action)) return {
          ok: false,
          error: 'auto_mode action must be start or stop'
        };
        if (action === 'stop') {
          AutoMode.stop('stopped by the Overseer');
          return {
            ok: true,
            running: false
          };
        ...
        if (!(AutoMode.settings.themes || []).length) {
          return {
            ok: false,
            error: 'auto mode has no themes configured — add them on the Dashboard, or use queue_art instead'
          };
        }
        AutoMode.start();
        return {
  ...
  const Overseer = {
    ...
    async init() {
      ...
      this._tick = setInterval(() => this.tick().catch(e => {
    ...
    snapshot() {
      ...
      const o = cfg();
      const learn = s.learn || {};
      const syncAge = learn.lastSyncAt ? Math.round((Date.now() - learn.lastSyncAt) / 36e5) : null;
      ...
      return {
        ...
        autoMode: window.AutoMode ? {
          running: !!AutoMode.running,
        ...
        stats: {
          lastSyncHoursAgo: syncAge,
          published: window.Insights ? Insights.scored().length : 0
        },
        chatImages: this.chatImages().slice(-8).map(a => ({
          id: a.id,
          name: a.name,
          size: a.w && a.h ? `${a.w}x${a.h}` : null,
          sentAsPixels: a.sent
        })),
        ...
        research: research ? {
          ...
          images: (research.images || []).length,
          promptSets: (research.promptSets || []).map(p => ({
            id: p.id,
    ...
    gateDescription() {
      ...
      return `score >= ${g.minScore}, no defect worse than "${g.maxDefect}"` + `${g.requireQcPass ? ', inspector verdict PASS' : ''}` + `${g.requireMetadata ? ', metadata written' : ''}` + `, at most ${g.maxPerRun} per run and ${g.maxPerDay} per day`;
    },
    sentToday() {
      const g = cfg().autoSubmit || {};
    ...
    gateCard(card) {
      ...
      if (card.status === 'drafted') return {
        ok: false,
        why: 'already uploaded'
      };
      if (card.status === 'qc_error' || card.qcAttempt) return {
        ok: false,
        why: 'the latest QC attempt is incomplete; retry inspection before uploading'
      };
      if (!card.qc) return {
        ok: false,
        why: 'never inspected'
      };
      if (g.requireQcPass && card.qc.verdict !== 'PASS') return {
        ok: false,
        why: `the inspector failed it (${card.qc.verdict})`
      };
      const score = Number(card.qc.score);
      if (!Number.isFinite(score) || score < 1 || score > 10) return {
        ok: false,
        why: 'invalid inspection score; retry QC'
      };
      if (score < (g.minScore || 8)) return {
        ok: false,
        why: `score ${score} is below ${g.minScore}`
      };
      ...
      if (bad.length) return {
        ok: false,
        why: `${bad.length} ${bad[0]} defect(s) in the inspection`
      };
      if (g.requireMetadata && !(card.metadata && card.metadata.title)) return {
        ok: false,
        why: 'no metadata written'
      };
      return {
        ok: true
      };
    },
    async webResearch({query: query, kind: kind = 'concept', webLimit: webLimit = 6, imageLimit: imageLimit = 6} = {}) {
      ...
      if (!window.ala?.research?.search) return {
        ok: false,
        error: 'the web research bridge is not loaded — restart the app to load this feature'
      };
      const type = kind === 'character' ? 'character' : 'concept';
      const found = await window.ala.research.search({
        query: q,
        kind: type,
        webLimit: clamp(webLimit, 3, 10),
        imageLimit: clamp(imageLimit, 1, 10)
      }).catch(e => ({
        ok: false,
        error: e.message,
        sources: [],
      ...
      if (!found?.ok) return {
        ok: false,
        error: found?.error || 'the web search returned nothing',
      ...
      const pack = {
        ...
        sources: sources,
        images: images,
        warnings: (found.warnings || []).map(x => String(x).slice(0, 300)).slice(0, 8),
        promptSets: []
      };
      ...
      return {
        ...
        note: images.length ? 'Reference ids are saved. Pass the useful ones to write_research_prompts; nothing has been queued.' : 'No image survived discovery, but the saved text sources can still ground prompt writing.'
      };
    },
    async writeResearchPrompts({researchId: researchId = '', imageIds: imageIds = null, count: count = 6, mode: mode = 'recreate', theme: theme = '', extra: extra = ''} = {}) {
      const epoch = this._cancelEpoch;
      if (this._abort) return cancelled();
      const pack = this.researchPack(researchId);
      if (!pack) return {
        ok: false,
        error: researchId ? `no saved web research called ${researchId} — run web_research again` : 'there is no saved web research yet — run web_research first'
      };
      if (!window.PromptLab?.fromResearch) return {
        ok: false,
        error: 'the research prompt writer is not loaded — restart the app'
      };
      ...
      if (selectedIds?.size && !wanted.length) {
        return {
          ...
          requestedImageIds: [ ...selectedIds ],
          availableImageIds: [ ...knownImageIds ]
        };
      }
      ...
      const briefs = [];
      const referenceErrors = [ ...selectedIds ? [ ...selectedIds ].filter(id => !knownImageIds.has(id)).map(id => ({
        id: id,
        error: 'no such image id in this research pack'
      })) : [], ...overCeiling ];
      let concurrency = 1;
      try {
      ...
      await U.mapLimit(selected, concurrency, async im => {
        ...
        try {
          ...
          const brief = pack.kind === 'character' ? await PromptLab.readCharacterReference({
            base64: small.base64,
            mime: small.mime,
            label: im.title,
            subject: pack.query
      ...
      if (!briefs.length && !(pack.sources || []).length) {
        return {
          ok: false,
          error: 'none of the references could be read and there are no text sources to fall back to',
      ...
      try {
        written = await PromptLab.fromResearch({
          ...
          mode: validModes.has(mode) ? mode : 'recreate',
        ...
        return {
          ok: false,
          error: `could not write researched prompts: ${e.message}`,
          failedReferences: referenceErrors
        };
      }
      if (this._abort || epoch !== this._cancelEpoch) return cancelled();
      ...
      if (!prompts.length) return {
        ok: false,
        error: 'the writer returned no usable prompts'
      };
      const set = {
        id: `prompts-${U.uid()}`,
        at: Date.now(),
      ...
      return {
        ...
        researchId: pack.id,
        promptSetId: set.id,
        written: prompts.length,
        prompts: prompts,
        referenceCount: briefs.length,
        failedReferences: referenceErrors,
        referenceCeiling: ceiling,
        skippedReferences: overCeiling.map(s => s.id),
    ...
    async queueResearchPrompts({promptSetId: promptSetId, researchId: researchId = '', theme: theme = '', referenceImages: referenceImages = null, first: first = false} = {}) {
      if (this._abort) return cancelled();
      const noGen = this.generationRefusal();
      if (noGen) return noGen;
      const chatRefs = this.chatReferenceRows(referenceImages);
      if (chatRefs.error) return {
        ok: false,
        error: chatRefs.error,
        availableImageIds: chatRefs.availableImageIds
      };
      const found = this.promptSet(promptSetId, researchId);
      if (!found) return {
        ok: false,
        error: 'that saved prompt set does not exist — use the promptSetId returned by write_research_prompts'
      };
      const {pack: pack, set: set} = found;
      ...
      const activeIndexes = new Set((State.queue || []).filter(j => [ 'queued', 'generating' ].includes(j.status) && String(j.researchId || '') === String(pack.id) && String(j.promptSetId || '') === String(set.id) && Number.isInteger(j.researchPromptIndex)).map(j => j.researchPromptIndex));
      const queuedNow = new Set((State.queue || []).filter(j => [ 'queued', 'generating' ].includes(j.status)).map(j => String(j.prompt || '').trim()));
      ...
      const jobs = unique.map(row => {
        ...
        if (chatRefs.rows.length) {
          job.chatReferences = chatRefs.rows.map(r => ({
            ...r
          }));
      ...
      return {
        ...
        running: !!Pipeline.running,
        batchId: batchId,
        position: first ? 'front' : 'back',
        jobsAhead: this.jobsAhead(batchId),
        referenceCount: (set.imageIds || []).length,
        ...Pipeline.running ? {} : {
          error: 'Prompts are queued, but the worker did not start; check the generation driver.'
        },
    ...
    editPrompt(instruction) {
      ...
      const change = String(instruction || '').trim().replace(/\s+/g, ' ').replace(/(?:[.\s]*\bchange nothing else)+[.\s]*$/i, '').replace(/[.\s]+$/, '');
    ...
    async editImage({image: image, instruction: instruction = '', count: count = 1, first: first = true} = {}) {
      if (this._abort) return cancelled();
      const noGen = this.generationRefusal();
      if (noGen) return noGen;
      ...
      if (!change) return {
        ok: false,
        error: 'edit_image needs an instruction — what should change in the picture?'
      };
      if (change.length > 1500) return {
        ok: false,
        error: 'the edit instruction must fit within 1500 characters'
      };
      const id = String(image || '').trim();
      if (!id) return {
        ok: false,
        error: 'name the picture to edit (image: a1…)',
        availableImageIds: this.chatImages().map(a => a.id)
      };
      const refs = this.chatReferenceRows([ id ]);
      if (refs.error) return {
        ok: false,
        error: refs.error,
        availableImageIds: refs.availableImageIds
      };
      const g = State.settings && State.settings.gen || {};
      if (g.engine !== 'comfy') return {
        ok: false,
        error: 'image edits need the ComfyUI engine with the Qwen-Image 2.1 reference workflow (Settings → Generation); Perchance cannot take a picture as input'
      };
      ...
      job.count = n;
      job.editOf = id;
      job.chatReferences = refs.rows.map(r => ({
        ...r
      }));
      ...
      return {
        ...
        ...Pipeline.running ? {} : {
          error: 'The edit is queued, but the worker did not start; check the generation driver.'
        }
      };
    },
    async queueArt({theme: theme, count: count, mode: mode = 'exploit', referenceImages: referenceImages = null, first: first = false} = {}) {
      const epoch = this._cancelEpoch;
      if (this._abort) return cancelled();
      const t = String(theme || '').trim();
      if (!t) return {
        ok: false,
        error: 'queue_art needs a theme — say what the pictures are about'
      };
      const noGen = this.generationRefusal();
      if (noGen) return noGen;
      ...
      try {
        ...
        return {
          ok: false,
          error: `could not write prompts: ${e.message}`
        };
      }
      ...
      if (!kept.length) return {
        ok: false,
        error: 'every prompt written was either unusable or hit the banned-wording list'
      };
      const prof = window.PromptStyle ? PromptStyle.profile() : null;
      ...
      return {
        ...
        jobsAhead: this.jobsAhead(batchId),
        ...refs.rows.length ? {
          referenceImages: refs.rows.map(r => r.id)
        } : {},
        ...Pipeline.running ? {} : {
          error: 'Prompts are queued, but the worker did not start; check the generation driver.'
        },
    ...
    editsOf(imageId) {
      ...
      const isEdit = x => (x.editOf === imageId || String(x.theme || '').startsWith(`Edit of ${imageId}:`)) && Array.isArray(x.chatReferences) && x.chatReferences.some(r => r && r.fname === att.fname);
      return {
    ...
    async awaitBatchCards(batchId, {minutes: minutes = 20} = {}) {
      const epoch = this._cancelEpoch;
      const t0 = Date.now();
      ...
      for (;;) {
        ...
        if (!open.length) {
          ...
          return cards.length ? {
            ...
            error: `batch ${batchId} has no finished picture (it failed, was cancelled, or the id is wrong)`
          };
        }
        if (!Pipeline.running) return {
          error: `batch ${batchId} is still waiting in the queue but the worker is stopped — start it first`
        };
        if (Date.now() - t0 > minutes * 6e4) return {
          error: `batch ${batchId} is still rendering after ${minutes} minutes; no video was made`
        };
        await U.sleep(3e3);
      }
    },
    recentResults() {
      ...
      for (const c of State.library || []) {
        ...
        out.push({
          ...
          editOf: edit,
          what: String(c.theme || '').slice(0, 70),
          hasVideo: !!(c.video && c.video.fname)
        });
    ...
    queueOrder(max = 12) {
      const groups = this.queueGroups().map((g, i) => ({
        group: i + 1,
        batchId: g.list[0].batchId || null,
        theme: String(g.list[0].theme || '').slice(0, 80),
        source: g.list[0].promptSource || '',
        jobs: g.list.length
      }));
      const out = groups.slice(0, max);
      if (groups.length > max) out.push({
        more: groups.length - max,
        jobs: groups.slice(max).reduce((n, g) => n + g.jobs, 0)
      });
      return out;
    },
    setQueueOrder(order) {
      ...
      if (!queued.length) return {
        ok: false,
        error: 'nothing is waiting in the queue'
      };
      const taken = new Set;
      const seq = [];
      const unknown = [];
      ...
      if (unknown.length) {
        return {
          ok: false,
          error: `could not match ${unknown.map(u => JSON.stringify(u)).join(', ')} — use group numbers or batchIds from queueOrder`,
          queueOrder: this.queueOrder()
        };
      }
    ...
    selectQueued({batchId: batchId = '', theme: theme = '', ids: ids = null} = {}) {
      ...
      if (Array.isArray(ids) && ids.length) {
        ...
        pick = newest.batchId ? j => j.batchId === newest.batchId : j => !j.batchId && j.theme === newest.theme && j.promptSource === newest.promptSource;
        ...
        return {
          error: 'say which jobs: batchId (or "latest" / "all"), theme, or ids',
          queueOrder: this.queueOrder()
        };
      }
      const jobs = queued.filter(pick);
      if (!jobs.length) return {
        error: `no queued job matches ${how}`,
        queueOrder: this.queueOrder()
      };
      return {
        jobs: jobs,
    ...
    reorderQueue({batchId: batchId = '', theme: theme = '', ids: ids = null, position: position = 'front', order: order = null} = {}) {
      if (Array.isArray(order) && order.length) return this.setQueueOrder(order);
      if (![ 'front', 'back' ].includes(position)) return {
        ok: false,
        error: 'position must be front or back'
      };
      const sel = this.selectQueued({
        batchId: batchId,
        theme: theme,
        ids: ids
      });
      if (sel.error) return {
        ...
        error: sel.error,
        ...sel.queueOrder ? {
          queueOrder: sel.queueOrder
        } : {}
      };
      const {jobs: moved, how: how, queued: queued} = sel;
      ...
      return {
        ...
        queueOrder: this.queueOrder(),
        note: position === 'front' ? `These run next.${generating ? ' The picture already rendering finishes first; nothing was cancelled.' : ''}` : 'These now run after everything else that is waiting.'
      };
    },
    cancelQueued({batchId: batchId = '', theme: theme = '', ids: ids = null} = {}) {
      const sel = this.selectQueued({
        batchId: batchId,
        theme: theme,
        ids: ids
      ...
      return {
        ...
        queueOrder: this.queueOrder(),
        ...generating ? {
          stillRendering: String(generating.theme || '').slice(0, 80)
        } : {},
        note: `Removed from the queue; nothing of theirs will be generated.${generating ? ' One job was already rendering and cannot be interrupted — it finishes and lands in Review, where it can be discarded.' : ''}`
      };
    },
    recentPrompts(n) {
    ...
    async waitForWorker({minutes: minutes = 20} = {}) {
      const epoch = this._cancelEpoch;
      const limit = clamp(minutes, 1, 180) * 6e4;
      const t0 = Date.now();
      while (Date.now() - t0 < limit) {
        if (this._abort || epoch !== this._cancelEpoch) {
          return {
            ...
            idle: false,
            stopped: true,
            waitedSeconds: Math.round((Date.now() - t0) / 1e3),
            note: 'the artist asked to stop, so the wait was cut short'
          };
        }
    ...
    async writeMetadata({ids: ids, filter: filter} = {}) {
      const epoch = this._cancelEpoch;
      if (this._abort) return cancelled();
      ...
      if (!cards.length) return {
        ok: true,
        written: 0,
        note: 'nothing was missing metadata'
      };
      let written = 0;
      const failed = [];
      for (const c of cards.slice(0, 24)) {
        ...
        try {
          await Pipeline.writeCardMetadata(c);
          written += 1;
        } catch (e) {
      ...
      return {
        ok: !stopped && !failed.length,
        written: written,
        failed: failed.length,
        firstError: failed[0] || null,
        ...stopped ? {
          cancelled: true,
          note: 'Stopped; metadata already in flight may have finished.'
        } : {}
      };
    },
    async approve({ids: ids} = {}) {
    ...
    async makeVideo({image: image, cardId: cardId, fromBatch: fromBatch = '', original: original = false, instructions: instructions = '', prompt: prompt = '', seconds: seconds = null, check: check = true} = {}) {
      const refused = this.generationRefusal();
      if (refused) return refused;
      if (!window.ComfyDriver || !window.ComfyUI) return {
        ok: false,
        error: 'the ComfyUI video engine is not loaded'
      };
      ...
      if (!imageId && !wantCard && !batchArg) {
        return {
          ok: false,
          error: 'name the picture: cardId / fromBatch for a picture you made, or image (his original attachment, a1…)',
          availableImageIds: this.chatImages().map(a => a.id)
        };
      }
      if (!wantCard && batchArg) {
        ...
        if (!bid) return {
          ok: false,
          error: 'there is no batch yet to take a picture from — pass cardId or image'
        };
        ...
        if (got.cards.length > 1) {
          return {
            ok: false,
            error: `batch ${bid} made ${got.cards.length} pictures — pass cardId for the one he means`,
            cards: got.cards.slice(0, 12).map(c => ({
              cardId: c.id,
              what: String(c.theme || '').slice(0, 60)
            }))
          };
        }
      ...
      if (!wantCard && imageId && original !== true) {
        ...
        if (edits.cards.length || edits.pending.length) {
          ...
          return {
            ok: false,
            refused: `${imageId} is his UNEDITED original, and it has been edited in this chat. "That image", "the generated one", "the edit" mean the edited picture: ` + (newest ? `call make_video with cardId "${newest.id}"` : `the edit is still rendering — call make_video with fromBatch "${edits.pending[0].batchId}"`) + `. Only if he explicitly wants the unedited original, call again with original:true.`,
      ...
      if (wantCard) {
        ...
        if (!card || !card.fname) return {
          ok: false,
          error: `no library card with id ${wantCard}`
        };
        ...
        if (found.error) return {
          ok: false,
          error: found.error,
          availableImageIds: found.availableImageIds
        };
      ...
      const direction = String(instructions || '').trim().slice(0, 2e3);
      const secs = seconds == null || seconds === '' ? null : Number(seconds);
      if (secs != null && !(secs > 0)) return {
        ok: false,
        error: `seconds must be a positive number, got "${seconds}"`
      };
      let text = String(prompt || '').trim();
      if (!text) {
        const written = await window.ComfyUI.writeVideoPrompt({
          base64: base64,
          mime: mime,
          userText: direction,
          mode: direction ? 'hybrid' : 'auto',
          seconds: secs
        });
        text = written.prompt;
      }
      ...
      const render = p => new window.ComfyDriver(State.settings).convertToVideo({
        base64: base64,
        mime: mime,
        prompt: p,
        seconds: secs,
      ...
      const attempts = [];
      if (checkOn) {
        verdict = await window.VideoCheck.check({
          fname: res.fname,
          request: request,
      ...
      if (!card) {
        ...
        card = {
          id: saved.id,
          jobId: null,
          theme: 'Overseer chat picture',
          prompt: '',
          promptSource: 'overseer-video',
          chatReferences: [ {
            id: att.id,
            fname: att.fname,
            name: String(att.name || '').slice(0, 80)
          } ],
          fname: saved.fname,
          path: saved.path,
          url: saved.url,
      ...
      const out = {
        ...
        cardId: card.id,
        video: res.fname,
        sizeMB: +(res.size / 1048576).toFixed(1),
        gif: !!res.gif,
        lengthSeconds: secs || 'workflow default',
        seconds: Math.round((Date.now() - t0) / 1e3),
        where: 'Review',
        prompt: text.slice(0, 4e3),
    ...
    async changePipeline(a = {}) {
      ...
      const given = [ 'passThreshold', 'skipQc', 'skipMetadata', 'maxRetries' ].filter(k => a[k] !== undefined && a[k] !== null);
      if (!given.length) {
        return {
          ok: true,
          current: before,
          note: 'nothing was changed — these are the current values'
        };
      }
      const patch = {};
      for (const k of given) {
        if (k === 'passThreshold') {
          ...
          if (!Number.isFinite(n)) return {
            ok: false,
            error: `passThreshold must be a number 1-10, got ${JSON.stringify(a[k])}`
          };
          ...
          if (!Number.isFinite(n)) return {
            ok: false,
            error: `maxRetries must be a number 0-5, got ${JSON.stringify(a[k])}`
          };
          patch[k] = clamp(n, 0, 5);
        } else {
          patch[k] = bool(a[k]);
      ...
      return {
        ok: true,
        changed: changed.length ? Object.fromEntries(changed.map(k => [ k, {
          from: before[k],
          to: after[k]
        } ])) : {},
        current: after,
    ...
    async submit({ids: ids} = {}) {
      ...
      if (kind === 'agent' && o.approval !== 'auto') {
        return {
          ok: false,
          refused: 'approval is set to "ask" — cards are left approved in Review for the artist to send'
        };
      }
      if (kind !== 'agent' && !this._uploadAsked) {
        return {
          ok: false,
          refused: 'nothing in this message asked for anything to be published — the cards are approved and waiting in Review'
        };
      }
      ...
      if (room <= 0) return {
        ok: false,
        refused: `the daily upload cap (${g.maxPerDay}) is already used up — ${sentToday} sent today`
      };
      ...
      for (const c of cards) {
        if (this._abort || epoch !== this._cancelEpoch) {
          blocked.push({
            id: c.id,
            why: 'cancelled before upload'
          });
          continue;
        }
        if (!(State.library || []).includes(c)) {
          blocked.push({
            id: c.id,
            why: 'card was removed'
          });
          continue;
        }
        if (sent.length >= room) {
        ...
        try {
          ...
          if (res ? res.ok !== true : c.status !== 'drafted') {
            blocked.push({
              id: c.id,
              why: `upload failed: ${res && res.error || c.error || 'no confirmed upload result'}`
            });
            continue;
          }
          const receipt = {
          ...
          if (res?.published === true || c.da?.published === true) published.push(receipt); else drafts.push({
            ...receipt,
      ...
      const stopped = this._abort || epoch !== this._cancelEpoch;
      return {
        ok: !stopped && !blocked.length && !drafts.some(d => d.publishError),
        sent: sent,
        published: published,
        drafts: drafts,
        blocked: blocked,
    ...
    history() {
      ...
      const out = [];
      slice.forEach((m, i) => {
        if (m.role === 'user') out.push(this.userContent(m, sendIds)); else if (m.role === 'agent') out.push({
          role: 'assistant',
          content: m.text
        }); else if (m.role === 'tool') {
          ...
          out.push({
            role: 'user',
            content: `RESULT of ${m.tool}${m.args ? ` with ${JSON.stringify(m.args)}` : ''}: ${body}`
          });
        }
      });
      return out;
    ...
    chatImages(messages = this.messages) {
      ...
      for (const m of messages || []) {
        if (m && m.role === 'user' && Array.isArray(m.attachments)) {
          for (const a of m.attachments) if (a && a.id && a.fname) all.push({
            ...a,
            at: m.at
          });
        }
      ...
      return all.map((a, i) => ({
        ...a,
        sent: i >= all.length - CHAT_IMAGES_SENT
      }));
    },
    userContent(m, sendIds) {
      ...
      for (const a of atts) {
        if (sendIds.has(a.id)) parts.push({
          type: 'ala_attachment',
          id: a.id,
          fname: a.fname,
          mime: a.mime
          ...
          type: 'text',
          text: `[${a.id} was shown earlier in this conversation and is not re-sent; rely on what you said about it.]`
        });
      }
      return {
        role: 'user',
        ...
        for (const p of msg.content) {
          ...
          try {
            ...
            out.push({
              type: 'image_url',
              image_url: {
                url: `data:${small.mime};base64,${small.base64}`
              }
            });
          } catch (e) {
            out.push({
              type: 'text',
              text: `[${p.id} could not be loaded (${String(e.message || e).slice(0, 80)}).]`
            });
          }
        }
    ...
    async shrinkForChat(base64, mime) {
      ...
      try {
        ...
        if (!longest || longest <= CHAT_IMAGE_EDGE) return {
          base64: base64,
          mime: mime
        };
        ...
        const out = canvas.toDataURL('image/jpeg', .9).split(',')[1];
        return out ? {
          base64: out,
          mime: 'image/jpeg'
        } : {
    ...
    chatReferenceRows(ids) {
      ...
      if (missing.length) {
        return {
          error: `no attached image called ${missing.join(', ')}`,
          availableImageIds: [ ...byId.keys() ]
        };
      }
      return {
        rows: [ ...new Set(want) ].map(id => {
          const a = byId.get(id);
          return {
            id: a.id,
            fname: a.fname,
    ...
    stopEverything() {
      this.cancelTurn();
      const stopped = [];
      if (window.Pipeline && Pipeline.running) {
        Pipeline.stop();
        stopped.push('the generation worker');
      }
      if (window.AutoMode && AutoMode.running) {
        AutoMode.stop('stopped by the artist');
        stopped.push('auto mode');
      }
      if (this.busy) {
        this._abort = true;
        stopped.push('what I was in the middle of');
      }
    ...
    async ask(text, attachments = []) {
      ...
      if (!files.length && this.STOP_RE.test(t)) {
        this.push('user', t);
        const {stopped: stopped, queued: queued} = this.stopEverything();
        this.push('agent', stopped.length ? `Stopped ${stopped.join(' and ')}.${this.busy ? ' No further actions will start; a request already in flight may still finish.' : ''}${queued ? ` ${queued} prompt(s) are still queued and will sit there until you start the worker again.` : ''}` : 'Nothing was running to stop.');
        this._dirty = true;
        await this.flush();
        State.emit('overseer', this);
        return;
      ...
      try {
        for (let i = 0; i < steps; i++) {
          ...
          if (reply.say) {
            ...
            if (reply.say !== lastSay) {
              this.push('agent', reply.say);
              lastSay = reply.say;
            }
          }
          if (!reply.tool) {
            if (!reply.tool && !run.tools.length && this.promiseUnkept(reply)) {
              if (!nudgeUsed && i + 1 < steps) {
                nudgeUsed = true;
                this.push('user', 'You said you would do something but called no tool. If it is something he ASKED for in his message, call the matching tool now. If it was an offer, a suggestion or a plan for later, do NOT do it — finish with done:true and tool:null.', {
                  hidden: true
                });
                continue;
              }
              this.push('note', `Said it would act but did not: "${String(reply.say).slice(0, 120)}"`, {
                kind: 'err'
              });
            }
            finished = true;
          ...
          if (this._abort) {
            this.push('note', 'Stopped part-way through, as asked.');
            break;
          }
          const tool = Object.prototype.hasOwnProperty.call(TOOLS, reply.tool) ? TOOLS[reply.tool] : null;
          if (!tool) {
            this.push('tool', JSON.stringify({
              error: `there is no tool called "${reply.tool}"`
            }), {
              tool: reply.tool
            });
            continue;
          ...
          if (!tool.reads && actions.has(key)) {
            result = {
              ok: false,
              refused: 'Duplicate action in this turn was not repeated. Use the previous result; ask the artist before retrying.'
            };
          } else {
        ...
        if (run.status !== 'done') {
          ...
          this.push('note', run.summary, {
            kind: this._abort ? '' : 'err'
          });
        }
      } catch (e) {
        ...
        this.push('note', `That did not work: ${e.message}`, {
          kind: 'err'
        });
    ...
    async step(kind) {
      const messages = [ {
        role: 'system',
        content: this.systemPrompt(kind)
      }, ...this.history(), {
        role: 'user',
      ...
      if (res.sawImages === false && !this._blindNoted) {
        ...
        this.push('note', `${res.provider || 'The model'} answered without seeing the attached image(s) — no vision-capable provider in the Overseer chain responded. Check Settings → Providers.`, {
          kind: 'err'
        });
      }
      let obj;
      try {
        ...
        return {
          say: String(res.text || '').trim().slice(0, 2e3),
          tool: null,
          done: true,
          usage: usage
        };
    ...
    slotsFor(ts = Date.now()) {
      ...
      const base = day.getTime() + from * 36e5;
      const slot = (to - from) * 36e5 / runs;
      const out = [];
      for (let i = 0; i < runs; i++) {
        const off = s.jitter ? slot * (.1 + Math.random() * .8) : slot / 2;
    ...
    async maybeAutoSync() {
      ...
      const every = clamp(a.everyMinutes, 15, 1440) * 6e4;
      ...
      log(res.ok ? `Overseer refreshed the DeviantArt numbers — ${res.items.length} deviation(s), no tokens spent.` : `Overseer could not refresh the numbers: ${res.error}`, res.ok ? '' : 'err');
    },
    async runCycle(reason = 'scheduled') {
      ...
      this.push('note', `${reason === 'scheduled' ? 'Scheduled run' : 'Run'} — ${(new Date).toLocaleString()}`, {
        marker: true
      });
      this.push('user', o.brief ? `It is ${(new Date).toLocaleString()}. This is your scheduled run. Read what has changed since last time and work on the standing brief. Report what you did.` : `It is ${(new Date).toLocaleString()}. This is your scheduled run, but no standing brief is set. Refresh the numbers, tell me anything worth knowing, and stop.`, {
        hidden: true
      });
      ...
      await this.ensureNext();
      const at = (cfg().schedule || {}).nextAt;
      window.ala.app.notify('AiLabor — Overseer', `${this.runs[this.runs.length - 1]?.summary || 'Run finished'}`.slice(0, 180)).catch?.(() => {});
