(function() {
  ...
  const PixivUI = {
    wire() {
      ...
      $('#btn-px-home').addEventListener('click', () => Pixiv.navigate('https://www.pixiv.net/illustration/create').catch(e => window.toast(e.message, 'err')));
      $('#btn-px-check').addEventListener('click', () => this.refreshStatus({
        loud: true,
        navigate: true
      }));
      ...
      State.on('pixivAuthLost', ({message: message}) => {
    ...
    async refreshStatus({loud: loud = false, navigate: navigate = false} = {}) {
      const hint = $('#pixiv-hint');
      if (hint && loud) hint.textContent = 'checking…';
      status = await Pixiv.status({
        navigate: navigate
      });
      this.render();
      if (loud) {
        window.toast(status.ok ? `Signed in to pixiv as ${status.username || 'user ' + status.userId}.` : 'Pixiv: ' + status.error, status.ok ? 'ok' : 'err');
      }
      return status;
    },
    async probe() {
      try {
        ...
        const groups = Object.entries(p.radios || {}).map(([k, v]) => `${k}: ${v.map(o => o.value + (o.checked ? '*' : '')).join(' / ')}`).join('\n');
        const reach = p.reach || {};
        const found = Object.keys(reach).filter(k => reach[k]);
        const lost = Object.keys(reach).filter(k => !reach[k]);
        const readiness = `Prepare can fill: ${found.join(', ') || 'nothing'}` + (lost.length ? `\nPrepare CANNOT find: ${lost.join(', ')} — those must be set by hand.` : '');
        State.addLog(`Pixiv form probe — ${(p.fields || []).length} named field(s), ` + `${found.length} of ${found.length + lost.length} fillable. Details in the console (window.__lastPixivProbe).`, lost.length ? 'err' : 'ok');
        note = `${readiness}\n\n${groups ? `Upload form values (a * marks the default):\n${groups}` : 'The page exposed no named radio groups — see the console dump.'}`;
        this.render();
      } catch (e) {
    ...
    render() {
      const hint = $('#pixiv-hint');
      if (hint) {
        hint.textContent = !status ? 'log in once — session persists' : status.ok ? `signed in as ${status.username || status.userId}` : status.error;
    ...
    renderStaged() {
      ...
      $('#btn-px-staged-check').addEventListener('click', async () => {
        ...
        if (res.found) window.toast(`Found it — "${card.metadata?.title || card.fname}" is on pixiv.`, 'ok'); else window.toast(res.ok ? 'Nothing new on your account yet — press Post first.' : 'Could not read your pixiv works: ' + (res.error || 'unknown'), 'err');
      });
    ...
    renderDiag() {
      ...
      if (!last) {
        body.innerHTML = head + 'Nothing posted yet this session. After a post — from the Drafts tab — ' + "the exact fields sent and pixiv's raw reply appear here.";
        return;
      }
      body.innerHTML = head + `\n        <div><b>${esc(fmtTime(last.at))}</b> · POST ${esc(last.endpoint)} · HTTP ${last.status ?? '—'}</div>\n        <pre class="px-pre">${esc(JSON.stringify({
        ...last.fields,
        'tags[]': last.tags
