(function() {
  const LAYOUTS = {
    ...
    'strip-v3': {
      label: 'Vertical strip — three wide panels',
      kind: 'page',
      slots: 3,
      rects: [ {
        x: 0,
    ...
    'strip-h3': {
      label: 'Horizontal strip — three tall panels',
      kind: 'page',
      slots: 3,
      rects: [ {
        x: 0,
    ...
    'hero-3': {
      label: 'Establishing shot + two reactions',
      kind: 'page',
      slots: 3,
      rects: [ {
        x: 0,
    ...
    'pair-v': {
      label: 'Before / after — two stacked',
      kind: 'page',
      slots: 2,
      rects: [ {
        x: 0,
    ...
    'story-left': {
      label: 'Picture left · story right',
      kind: 'story',
      slots: 1,
      rects: [ {
        x: 0,
    ...
    'story-right': {
      label: 'Picture right · story left',
      kind: 'story',
      slots: 1,
      rects: [ {
        x: .5,
    ...
    'story-top': {
      label: 'Picture above · story below',
      kind: 'story',
      slots: 1,
      rects: [ {
        x: 0,
  ...
  function layoutFor(templateId, panelCount) {
    ...
    if (!t) return {
      ...
      kind: 'page',
      slots: panelCount,
      rects: autoRects(panelCount)
    };
    if (panelCount > t.slots && t.kind === 'page') {
      return {
        ...t,
        label: `${t.label} (+${panelCount - t.slots} — auto grid)`,
        slots: panelCount,
        rects: autoRects(panelCount)
      };
    }
  ...
  function panelBoxes(layout, geom) {
    const {width: width, height: height, margin: margin, gutter: gutter} = geom;
  ...
  function fitFontSize(text, box, {max: max = 34, min: min = 11, lineHeight: lineHeight = 1.38, measureAt: measureAt}) {
    for (let size = max; size >= min; size--) {
      const lines = wrapText(text, box.w, s => measureAt(s, size));
      if (lines.length * size * lineHeight <= box.h) return {
        size: size,
        lines: lines,
        clipped: false
      };
  ...
  function drawCaption(ctx, panel, text, style, offsetY = 0) {
    ...
    const pad = Math.round(style.fontSize * .5);
    const maxW = Math.min(panel.w * .62, panel.w - pad * 2);
    ...
    const y = style.corner === 'bottom' ? panel.y + panel.h - boxH - pad - offsetY : panel.y + pad + offsetY;
    ...
    lines.forEach((line, i) => ctx.fillText(line, x + pad, y + pad * .8 + i * lh));
    return {
      x: x,
      y: y,
      w: boxW,
  ...
  function drawBubble(ctx, panel, bubble, style, {avoid: avoid = null} = {}) {
    const text = String(bubble.text || '').trim();
    if (!text) return null;
    const pad = Math.round(style.fontSize * .62);
    const maxW = Math.min(panel.w * .55, 420);
    ...
    const infX = bubble.kind === 'thought' ? boxW * .16 : 0;
    const infY = bubble.kind === 'thought' ? boxH * .28 : 0;
    const edge = 6;
    ...
    const ax = panel.x + (bubble.anchor?.x ?? .5) * panel.w;
    const ay = panel.y + (bubble.anchor?.y ?? .18) * panel.h;
    const x = span(minX, maxX, ax - boxW / 2);
    let y = span(minY, maxY, ay - boxH / 2);
    if (avoid && x < avoid.x + avoid.w && x + boxW > avoid.x && y - infY < avoid.y + avoid.h && y + boxH + infY > avoid.y) {
      y = span(minY, maxY, avoid.y + avoid.h + infY + Math.round(style.fontSize * .5));
    }
    const tx = panel.x + (bubble.tailTo?.x ?? (bubble.anchor?.x ?? .5)) * panel.w;
    const ty = panel.y + (bubble.tailTo?.y ?? Math.min(1, (bubble.anchor?.y ?? .18) + .34)) * panel.h;
    ctx.fillStyle = style.bg;
    ctx.strokeStyle = style.borderColor;
    ...
    if (bubble.kind === 'thought') {
      ...
      for (let i = 1; i <= steps; i++) {
        const t = i / (steps + 1);
        const r = style.fontSize * (.34 - t * .16);
        ctx.beginPath();
      ...
      roundRect(ctx, x, y, boxW, boxH, style.radius ?? Math.round(style.fontSize * .9));
      ctx.fill();
      const half = Math.min(boxW * .12, style.fontSize * .8);
      ...
      ctx.fill();
      ctx.beginPath();
      ctx.moveTo(cx - half, baseY + dir * .5);
      ctx.lineTo(tx, ty);
      ctx.lineTo(cx + half, baseY + dir * .5);
      ctx.stroke();
      roundRect(ctx, x, y, boxW, boxH, style.radius ?? Math.round(style.fontSize * .9));
      ctx.stroke();
    }
    ...
    lines.forEach((line, i) => {
      ...
      ctx.fillText(line, x + (boxW - w) / 2, y + pad * .7 + i * lh);
    });
    if (bubble.speaker && bubble.kind !== 'thought') {
      const size = Math.round(style.fontSize * .68);
      ctx.font = `600 ${size}px ${style.font}`;
      const tw = ctx.measureText(bubble.speaker).width;
      const cpad = Math.round(size * .45);
      const cw = tw + cpad * 2;
      const ch = Math.round(size * 1.5);
      const cx = span(panel.x + edge, panel.x + panel.w - cw - edge, x + pad);
      const cy = y - ch - Math.round(size * .25) >= panel.y + edge ? y - ch - Math.round(size * .25) : y + boxH + Math.round(size * .25);
      ctx.fillStyle = style.bg;
      ctx.strokeStyle = style.borderColor;
      ctx.lineWidth = Math.max(1, (style.borderWidth || 2) * .7);
      roundRect(ctx, cx, cy, cw, ch, Math.round(ch / 2));
      ctx.fill();
      ctx.stroke();
      ctx.fillStyle = style.speakerColor || style.color;
      ctx.textBaseline = 'top';
      ctx.fillText(bubble.speaker, cx + cpad, cy + Math.round(size * .22));
    }
    return {
      x: x,
      y: y,
